...
import com.kaiwudb.util.WriterHandler;
...
public class Driver implements java.sql.Driver {
  ...
  private static final Logger PARENT_LOGGER = Logger.getLogger("com.kaiwudb");
  ...
  private static final String DEFAULT_PORT =
      /*$"\""+mvn.project.property.template.default.pg.port+"\";"$*//*-*/"26257";
  ...
  private Properties loadDefaultProperties() throws IOException {
    ...
    try {
      KWProperty.USER.set(merged, System.getProperty("user.name"));
    ...
    if (cl == null) {
      LOGGER.log(Level.WARNING, "Can't find a classloader for the Driver; not loading driver "
          + "configuration from com/kaiwudb/driverconfig.properties");
    ...
    Enumeration<URL> urlEnum = cl.getResources("com/kaiwudb/driverconfig.properties");
   ...
   * probably want to have set up the KaiwuDB database itself to use the same encoding, with the
   * {@code -E <encoding>} argument to createdb.</p>
   *
   * <p>Our protocol takes the forms:</p>
   *
   * <pre>
   *  jdbc:kaiwudb://host:port/database?param1=val1&amp;...
   * </pre>
   *
  ...
  public Connection connect(String url, Properties info) throws SQLException {
    ...
    if (!url.startsWith("jdbc:kaiwudb:")) {
      return null;
    }
    try {
      defaults = getDefaultProperties();
    } catch (IOException ioe) {
      throw new KSQLException(GT.tr("Error loading default settings from driverconfig.properties"),
          KSQLState.UNEXPECTED_ERROR, ioe);
    }
    ...
    try {
      ...
      if (timeout <= 0) {
        ...
        setPropertiesToInfo(info, props);
        return conn;
      }

      ConnectThread ct = new ConnectThread(url, props);
      Thread thread = new Thread(ct, "KaiwuDB JDBC driver connection thread");
      ...
      // com.kaiwudb.unusual error will be returned instead.
      throw ex1;
      ...
      throw new KSQLException(
          ...
          KSQLState.UNEXPECTED_ERROR, ace);
    } catch (Exception ex2) {
      LOGGER.log(Level.FINE, "Unexpected connection error: ", ex2);
      throw new KSQLException(
          GT.tr(
              "Something unusual has occurred to cause the driver to fail. Please report this exception."),
          KSQLState.UNEXPECTED_ERROR, ex2);
    }
  }

  private static void setPropertiesToInfo(Properties info, Properties props) {
    // expand the KWDB connection parameters.
    if (info != null) {
      Set<String> e = props.stringPropertyNames();
      for (String propName : e) {
   ...
   *
   * <p>See {@link KWProperty#LOGGER_FILE} and {@link KWProperty#LOGGER_FILE}</p>
   *
   * @param props Connection Properties
   */
  private void setupLoggerFromProperties(final Properties props) {
    final String driverLogLevel = KWProperty.LOGGER_LEVEL.get(props);
    ...
    final String driverLogFile = KWProperty.LOGGER_FILE.get(exprProps);
    ...
    public Connection getResult(long timeout) throws SQLException {
      ...
      synchronized (this) {
        while (true) {
          ...
          if (resultException != null) {
            if (resultException instanceof SQLException) {
              ...
              throw new KSQLException(
                  GT.tr(
                      "Something unusual has occurred to cause the driver to fail. Please report this exception."),
                  KSQLState.UNEXPECTED_ERROR, resultException);
            }
          }
          ...
          if (delay <= 0) {
            abandoned = true;
            throw new KSQLException(GT.tr("Connection attempt timed out."),
  ...
  private static Connection makeConnection(String url, Properties props) throws SQLException {
    return new KwConnection(hostSpecs(props), user(props), database(props), props, url);
  }
   ...
   * don't. Our protocols start with jdbc:kaiwudb:
   *
  ...
  public DriverPropertyInfo[] getPropertyInfo(String url, Properties info) {
    ...
    KWProperty[] knownProperties = KWProperty.values();
   ...
   *
   * <p>For KaiwuDB, this is not yet possible, as we are not SQL92 compliant (yet).</p>
   */
  @Override
  ...
  public static Properties parseURL(String url, Properties defaults) {
    ...
    if (!urlServer.startsWith("jdbc:kaiwudb:")) {
      LOGGER.log(Level.FINE, "JDBC URL must start with \"jdbc:kaiwudb:\" but was: {0}", url);
      return null;
    }
    urlServer = urlServer.substring("jdbc:kaiwudb:".length());
    ...
    if (urlServer.startsWith("//")) {
      ...
      urlProps.setProperty("KWDBNAME", URLCoder.decode(urlServer.substring(slash + 1)));
      ...
      urlProps.setProperty("KWPORT", ports.toString());
      urlProps.setProperty("KWHOST", hosts.toString());
    } else {
      /*
       if there are no defaults set or any one of PORT, HOST, DBNAME not set
       then set it to default
      */
      if (defaults == null || !defaults.containsKey("KWPORT")) {
        urlProps.setProperty("KWPORT", DEFAULT_PORT);
      }
      if (defaults == null || !defaults.containsKey("KWHOST")) {
        urlProps.setProperty("KWHOST", "localhost");
      }
      if (defaults == null || !defaults.containsKey("KWDBNAME")) {
        urlProps.setProperty("KWDBNAME", URLCoder.decode(urlServer));
      }
    }
  ...
  private static HostSpec[] hostSpecs(Properties props) {
    String[] hosts = props.getProperty("KWHOST").split(",");
    String[] ports = props.getProperty("KWPORT").split(",");
  ...
  private static String database(Properties props) {
    return props.getProperty("KWDBNAME", "");
  }
  ...
  private static long timeout(Properties props) {
    String timeout = KWProperty.LOGIN_TIMEOUT.get(props);
    if (timeout != null) {
      try {
   ...
   * @return KSQLException with a localized message giving the complete description of the
