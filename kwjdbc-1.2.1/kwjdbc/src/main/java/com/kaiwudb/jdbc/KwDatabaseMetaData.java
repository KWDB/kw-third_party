...
import com.kaiwudb.util.JdbcBlackHole;
...
public class KwDatabaseMetaData implements DatabaseMetaData {
  private static final Logger LOGGER = Logger.getLogger("KwDatabaseMetaData");

  public KwDatabaseMetaData(KwConnection conn) {
    this.connection = conn;
  }

  private String keywords;

  protected final KwConnection connection; // The connection association
...
//        rs = stmt.executeQuery(sql);
//        if (!rs.next()) {
//          throw new KSQLException(GT.tr("Unable to find name datatype in the system catalogs."),
//              KSQLState.UNEXPECTED_ERROR);
//        }
...
//    return nameDataLength - 1;
  ...
  protected boolean isTimeSeries() {
    ...
    String sql = "select distinct engine_type from information_schema.schemata where catalog_name = (select current_database());";
    Statement stmt = null;
    ResultSet rs = null;
    try {
      ...
      rs = stmt.executeQuery(sql);
      if (rs != null && rs.next()) {
        String databaseType = rs.getString(1);
        if ("TIME SERIES".equals(databaseType)) {
          timeSeries = true;
        }
      }
    } catch (SQLException e) {
      LOGGER.log(Level.WARNING, "Get current Database type failed, cause by: {0}", e.getMessage());
    } finally {
      JdbcBlackHole.close(rs);
      JdbcBlackHole.close(stmt);
    }
  ...
  /**
   * Retrieves the name of this database product. We hope that it is KaiwuDB, so we return that
   * explicitly.
   *
   * @return "KaiwuDB"
   */
   ...
   * {@inheritDoc}
   *
   * <p>From KaiwuDB 9.0+ return the keywords from pg_catalog.pg_get_keywords()</p>
   *
    ...
    if (keywords == null) {
      if (connection.haveMinimumServerVersion(ServerVersion.v9_0)) {
        ...
        try {
          ...
          rs = stmt.executeQuery(sql);
          if (!rs.next()) {
            throw new KSQLException(GT.tr("Unable to find keywords in the system catalogs."),
                KSQLState.UNEXPECTED_ERROR);
          }
        ...
        // Static list from KW8.2 src/backend/parser/keywords.c with SQL:2003 excluded.
  ...
  public String getSearchStringEscape() throws SQLException {
    ...
    // would be fed directly into kw's input parser so it would
   ...
   * {@inheritDoc}
   *
   * <p>KaiwuDB allows any high-bit character to be used in an unquoted identifier, so we can't
   ...
   * {@inheritDoc}
   *
   * @return true if connected to KaiwuDB 6.1+
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * @return true if connected to KaiwuDB 7.3+
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * @return true if connected to KaiwuDB 6.4+
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * @return true if connected to KaiwuDB 7.1+
   */
  @Override
...
//    return true;
  ...
  /**
   * {@inheritDoc}
   * <p>KaiwuDB doesn't have schemas, but when it does, we'll use the term "schema".</p>
   *
   * @return {@code "schema"}
   */
  @Override
...
//    return true;
   ...
   * {@inheritDoc}
   *
   * @return true if connected to KaiwuDB 6.5+
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * @return true if connected to KaiwuDB 6.3+
   */
  @Override
  ...
  /**
   * {@inheritDoc} In KaiwuDB, Cursors are only open within transactions.
   */
  @Override
...
//    return true;
  ...
  public ResultSet getProcedureColumns(String catalog, String schemaPattern,
    ...
    while (rs.next()) {
      ...
      if (returnTypeType.equals("b") || returnTypeType.equals("d") || returnTypeType.equals("e")
        ...
        tuple[6] = connection.encodeString(connection.getTypeInfo().getKWType(returnType));
        tuple[7] = null;
        tuple[8] = null;
        tuple[9] = null;
        tuple[10] = null;
      ...
      for (int i = 0; i < numArgs; i++) {
        ...
        tuple[6] = connection.encodeString(connection.getTypeInfo().getKWType(argOid));
        tuple[7] = null;
        tuple[8] = null;
        tuple[9] = null;
        tuple[10] = null;
      ...
      if (returnTypeType.equals("c") || (returnTypeType.equals("p") && argModesArray != null)) {
        ...
        while (columnrs.next()) {
          ...
          tuple[6] = connection.encodeString(connection.getTypeInfo().getKWType(columnTypeOid));
          tuple[7] = null;
          tuple[8] = null;
          tuple[9] = null;
          tuple[10] = null;
  ...
  /**
   * KaiwuDB does not support multiple catalogs from a single connection, so to reduce confusion
  ...
  public ResultSet getColumns(String catalog, String schemaPattern, String tableNamePattern,
    ...
    while (rs.next()) {
      ...
      String kwType = connection.getTypeInfo().getKWType(typeOid);
      tuple[5] = connection.encodeString(kwType); // Type name
      ...
      if (kwType.equals("bit") || kwType.equals("varbit")) {
...
//    return ((BaseStatement) createMetaDataStatement()).createDriverResultSet(f, v);
  ...
  public ResultSet getBestRowIdentifier(String catalog, String schema, String table,
    ...
    while (rs.next()) {
      ...
      tuple[3] = connection.encodeString(connection.getTypeInfo().getKWType(typeOid));
  ...
  public ResultSet getVersionColumns(String catalog, String schema, String table)
    ...
    /*
     * KaiwuDB does not have any column types that are automatically updated like some databases'
  ...
  protected ResultSet getImportedExportedKeys(String primaryCatalog, String primarySchema,
    ...
    // Starting in KaiwuDB 9.0, pg_constraint was augmented with the conindid column, which
...
//    return true;
  ...
  public ResultSet getUDTs(String catalog, String schemaPattern, String typeNamePattern,
    ...
    for (Iterator<String> i = connection.getTypeInfo().getKWTypeNamesWithSQLTypes(); i.hasNext(); ) {
      String kwType = i.next();
      int sqlType = connection.getTypeInfo().getSQLType(kwType);
      sqlwhen.append(" when typname = ").append(escapeQuotes(kwType)).append(" then ").append(sqlType);
    }
...
//    return true;
  ...
  public ResultSet getFunctions(String catalog, String schemaPattern, String functionNamePattern)
    ...
    // The kw_get_function_result only exists 8.4 or later
    boolean kwFuncResultExists = connection.haveMinimumServerVersion(ServerVersion.v8_4);

    // Use query that support kw_get_function_result to get function result, else unknown is defaulted
    String funcTypeSql = DatabaseMetaData.functionResultUnknown + " ";
    if (kwFuncResultExists) {
      funcTypeSql = " CASE "
