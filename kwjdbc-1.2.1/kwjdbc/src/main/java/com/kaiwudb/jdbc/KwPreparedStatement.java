...
import com.kaiwudb.core.v3.BatchedQuery;
...
import com.kaiwudb.util.KWTime;
import com.kaiwudb.util.KWTimestamp;
import com.kaiwudb.util.KWobject;
import com.kaiwudb.util.ReaderInputStream;
...
class KwPreparedStatement extends KwStatement implements PreparedStatement {
  ...
  KwPreparedStatement(KwConnection connection, String sql, int rsType, int rsConcurrency,
                      int rsHoldability) throws SQLException {
    this(connection, connection.borrowQuery(sql), rsType, rsConcurrency, rsHoldability);
  }

  KwPreparedStatement(KwConnection connection, CachedQuery query, int rsType,
  ...
  public void closeImpl() throws SQLException {
    if (preparedQuery != null) {
      ((KwConnection) connection).releaseQuery(preparedQuery);
    }
  }
  ...
  public void setNull(int parameterIndex, int sqlType) throws SQLException {
    ...
    switch (sqlType) {
      ...
      default:
        // Bad Types value.
        throw new KSQLException(GT.tr("Unknown Types value."), KSQLState.INVALID_PARAMETER_TYPE);
    }
  ...
  private void setCharacterStreamPost71(int parameterIndex, InputStream x, int length,
    ...
    if (length < 0) {
      throw new KSQLException(GT.tr("Invalid stream length {0}.", length),
        KSQLState.INVALID_PARAMETER_VALUE);
    }

    // Version 7.2 supports AsciiStream for all KW text types (char, varchar, text)
    // As the spec/javadoc for this method indicate this is to be used for
    // large String values (i.e. LONGVARCHAR) KW doesn't have a separate
    ...
    try {
      ...
      throw new KSQLException(GT.tr("The JVM claims not to support the {0} encoding.", encoding),
        KSQLState.UNEXPECTED_ERROR, uee);
    } catch (IOException ioe) {
      throw new KSQLException(GT.tr("Provided InputStream failed."), KSQLState.UNEXPECTED_ERROR,
        ioe);
    }
  }
  ...
  public void setBinaryStream(int parameterIndex, InputStream x, int length) throws SQLException {
    ...
    // Version 7.2 supports BinaryStream for for the KW bytea type
    // As the spec/javadoc for this method indicate this is to be used for
  ...
  // Helper method for setting parameters to KWobject subclasses.
  private void setKWobject(int parameterIndex, KWobject x) throws SQLException {
    String typename = x.getType();
    int oid = connection.getTypeInfo().getKWType(typename);
    if (oid == Oid.UNSPECIFIED) {
      throw new KSQLException(GT.tr("Unknown type {0}.", typename),
        KSQLState.INVALID_PARAMETER_TYPE);
    }

    if ((x instanceof KWBinaryObject) && connection.binaryTransferSend(oid)) {
      KWBinaryObject binObj = (KWBinaryObject) x;
  ...
  private void setMap(int parameterIndex, Map<?, ?> x) throws SQLException {
    int oid = connection.getTypeInfo().getKWType("hstore");
    if (oid == Oid.UNSPECIFIED) {
      throw new KSQLException(GT.tr("No hstore extension installed."),
        KSQLState.INVALID_PARAMETER_TYPE);
    }
    ...
    switch (targetSqlType) {
      case Types.SQLXML:
        if (in instanceof SQLXML) {
          ...
          setSQLXML(parameterIndex, new KwSQLXML(connection, in.toString()));
        }
        break;
      case Types.INTEGER:
      ...
      case Types.TIMESTAMP_WITH_TIMEZONE:
        ...
        } else if (in instanceof KWTimestamp) {
          setObject(parameterIndex, in);
        } else {
          throw new KSQLException(
      ...
      case Types.DISTINCT:
        ...
        break;
      case Types.OTHER:
        if (in instanceof KWobject) {
          setKWobject(parameterIndex, (KWobject) in);
        ...
        break;
      default:
        throw new KSQLException(GT.tr("Unsupported Types value: {0}", targetSqlType),
          KSQLState.INVALID_PARAMETER_TYPE);
    }
  }
  ...
  private static KSQLException cannotCastException(final String fromType, final String toType) {
  ...
  private static KSQLException cannotCastException(final String fromType, final String toType,
    ...
    return new KSQLException(
      ...
      KSQLState.INVALID_PARAMETER_TYPE, cause);
  }
  ...
  public void setObject(int parameterIndex, Object x) throws SQLException {
    ...
    if (x == null) {
    ...
    } else if (x instanceof KWobject) {
      setKWobject(parameterIndex, (KWobject) x);
      ...
      throw new KSQLException(GT.tr(
        ...
        x.getClass().getName()), KSQLState.INVALID_PARAMETER_TYPE);
    }
  }
  ...
  public ResultSetMetaData getMetaData() throws SQLException {
    ...
    if (rs == null || ((KwResultSet) rs).isResultSetClosed()) {
  ...
  public void setArray(int i, java.sql.Array x) throws SQLException {
    ...
    int oid = connection.getTypeInfo().getKWArrayType(typename);
    if (oid == Oid.UNSPECIFIED) {
      throw new KSQLException(GT.tr("Unknown type {0}.", typename),
        KSQLState.INVALID_PARAMETER_TYPE);
    }

    if (x instanceof KwArray) {
      KwArray arr = (KwArray) x;
      if (arr.isBinary()) {
  ...
  protected long createBlob(int i, InputStream inputStream, long length) throws SQLException {
    ...
    try {
      ...
      throw new KSQLException(GT.tr("Unexpected error writing large object to database."),
        KSQLState.UNEXPECTED_ERROR, se);
    } finally {
      try {
        outputStream.close();
      } catch (Exception e) {
  ...
  private String readerToString(Reader value, int maxLength) throws SQLException {
    try {
      ...
      throw new KSQLException(GT.tr("Provided Reader failed."), KSQLState.UNEXPECTED_ERROR, ioe);
    }
  }
  ...
  public void setCharacterStream(int i, java.io.Reader x, int length) throws SQLException {
    ...
    // Version 7.2 supports CharacterStream for for the KW text types
    // As the spec/javadoc for this method indicate this is to be used for
  ...
  public void setNull(int parameterIndex, int t, String typeName) throws SQLException {
    ...
    int oid = typeInfo.getKWType(typeName);
  ...
  public void setTime(int i, Time t, java.util.Calendar cal) throws SQLException {
    ...
    // If a KWTime is used, we can define the OID explicitly.
    if (t instanceof KWTime) {
      KWTime kwTime = (KWTime) t;
      if (kwTime.getCalendar() == null) {
        oid = Oid.TIME;
      } else {
        oid = Oid.TIMETZ;
        cal = kwTime.getCalendar();
      }
    }
  ...
  public void setTimestamp(int i, Timestamp t, java.util.Calendar cal) throws SQLException {
    ...
    // If a KWTimestamp is used, we can define the OID explicitly.
    if (t instanceof KWTimestamp) {
      KWTimestamp kwTimestamp = (KWTimestamp) t;
      if (kwTimestamp.getCalendar() == null) {
        oid = Oid.TIMESTAMP;
      } else {
        oid = Oid.TIMESTAMPTZ;
        cal = kwTimestamp.getCalendar();
      }
    }
    if (cal == null) {
      cal = getDefaultCalendar();
  ...
  public ParameterMetaData createParameterMetaData(BaseConnection conn, int[] oids)
    throws SQLException {
    return new KwParameterMetaData(conn, oids);
  }
  ...
  public void setBinaryStream(int parameterIndex, InputStream value, long length)
    ...
    if (length > Integer.MAX_VALUE) {
      throw new KSQLException(GT.tr("Object is too large to send over the protocol."),
        KSQLState.NUMERIC_CONSTANT_OUT_OF_RANGE);
    }
