...
import com.kaiwudb.jdbc2.ArrayAssistantRegistry;
 ...
 */
public class KwArray implements java.sql.Array {
  ...
  /**
   * Array list implementation specific for storing KW array elements.
   */
  private static class KwArrayList extends ArrayList<Object> {
  ...
  /**
   * Value of field as {@link KwArrayList}. Will be initialized only once within
   * {@link #buildArrayList()}.
   */
  protected KwArrayList arrayList;

  protected byte[] fieldBytes;

  private KwArray(BaseConnection connection, int oid) throws SQLException {
   ...
   */
  public KwArray(BaseConnection connection, int oid, String fieldString) throws SQLException {
    this(connection, oid);
   ...
   * @throws SQLException if something wrong happens
   */
  public KwArray(BaseConnection connection, int oid, byte[] fieldBytes) throws SQLException {
    this(connection, oid);
  ...
  public Object getArrayImpl(long index, int count, Map<String, Class<?>> map) throws SQLException {
    ...
    if (index < 1) {
      throw new KSQLException(GT.tr("The array index is out of range: {0}", index),
  ...
  private synchronized void buildArrayList() throws SQLException {
    ...
    if (fieldString != null) {
      ...
      // value occurred
      List<KwArrayList> dims = new ArrayList<KwArrayList>(); // array dimension arrays
      KwArrayList curArray = arrayList; // currently processed array
      ...
      for (int i = startOffset; i < chars.length; i++) {
        ...
        if (chars[i] == '\\') {
          ...
          if (dims.isEmpty()) {
            ...
            KwArrayList a = new KwArrayList();
            KwArrayList p = dims.get(dims.size() - 1);
            p.add(a);
            dims.add(a);
          }
   ...
   */
  private Object buildArray(KwArrayList input, int index, int count) throws SQLException {
    ...
    // array elements type
    final int type =
        connection.getTypeInfo().getSQLType(connection.getTypeInfo().getKWArrayElement(oid));
    ...
    if (type == Types.BIT) {
      ...
      for (; count > 0; count--) {
        ...
        if (dims > 1 || useObjects) {
          oa[length++] = o == null ? null
            : (dims > 1 ? buildArray((KwArrayList) o, 0, -1) : BooleanTypeUtil.castToBoolean((String) o));
        } else {
      ...
      for (; count > 0; count--) {
        ...
        if (dims > 1 || useObjects) {
          oa[length++] = o == null ? null
              : (dims > 1 ? buildArray((KwArrayList) o, 0, -1) : KwResultSet.toShort((String) o));
        } else {
          pa[length++] = o == null ? 0 : KwResultSet.toShort((String) o);
        }
      }
      ...
      for (; count > 0; count--) {
        ...
        if (dims > 1 || useObjects) {
          oa[length++] = o == null ? null
              : (dims > 1 ? buildArray((KwArrayList) o, 0, -1) : KwResultSet.toInt((String) o));
        } else {
          pa[length++] = o == null ? 0 : KwResultSet.toInt((String) o);
        }
      }
      ...
      for (; count > 0; count--) {
        ...
        if (dims > 1 || useObjects) {
          oa[length++] = o == null ? null
              : (dims > 1 ? buildArray((KwArrayList) o, 0, -1) : KwResultSet.toLong((String) o));
        } else {
          pa[length++] = o == null ? 0L : KwResultSet.toLong((String) o);
        }
      }
      ...
      for (; count > 0; count--) {
        ...
        oa[length++] = dims > 1 && v != null ? buildArray((KwArrayList) v, 0, -1)
            : (v == null ? null : KwResultSet.toBigDecimal((String) v));
      }
      ...
      for (; count > 0; count--) {
        ...
        if (dims > 1 || useObjects) {
          oa[length++] = o == null ? null
              : (dims > 1 ? buildArray((KwArrayList) o, 0, -1) : KwResultSet.toFloat((String) o));
        } else {
          pa[length++] = o == null ? 0f : KwResultSet.toFloat((String) o);
        }
      }
      ...
      for (; count > 0; count--) {
        ...
        if (dims > 1 || useObjects) {
          oa[length++] = o == null ? null
              : (dims > 1 ? buildArray((KwArrayList) o, 0, -1) : KwResultSet.toDouble((String) o));
        } else {
          pa[length++] = o == null ? 0d : KwResultSet.toDouble((String) o);
        }
      }
      ...
      for (; count > 0; count--) {
        Object v = input.get(index++);
        oa[length++] = dims > 1 && v != null ? buildArray((KwArrayList) v, 0, -1) : v;
      }
  ...
  public String getBaseTypeName() throws SQLException {
    buildArrayList();
    int elementOID = connection.getTypeInfo().getKWArrayElement(oid);
    return connection.getTypeInfo().getKWType(elementOID);
  }
  ...
  public ResultSet getResultSetImpl(long index, int count, Map<String, Class<?>> map)
    ...
    if (arrayList.dimensionsCount <= 1) {
      // array element type
      final int baseOid = connection.getTypeInfo().getKWArrayElement(oid);
      fields[0] = new Field("INDEX", Oid.INT4);
      ...
      for (int i = 0; i < count; i++) {
        ...
        t[0] = connection.encodeString(Integer.toString(offset + 1));
        t[1] = v == null ? null : connection.encodeString(toString((KwArrayList) v));
        rows.add(t);
      }
    }
  ...
  /**
   * Convert array list to KW String representation (e.g. {0,1,2}).
   */
  private String toString(KwArrayList list) throws SQLException {
    if (list == null) {
      return "NULL";
    }
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      if (v == null) {
        b.append("NULL");
      } else if (v instanceof KwArrayList) {
        b.append(toString((KwArrayList) v));
      } else {
