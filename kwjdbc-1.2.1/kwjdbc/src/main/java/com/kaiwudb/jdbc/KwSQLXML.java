...
public class KwSQLXML implements SQLXML {
  ...
  public KwSQLXML(BaseConnection conn) {
    this(conn, null, false);
  }

  public KwSQLXML(BaseConnection conn, String data) {
    this(conn, data, true);
  }

  private KwSQLXML(BaseConnection conn, String data, boolean initialized) {
    this.conn = conn;
    this.data = data;
  ...
  public synchronized InputStream getBinaryStream() throws SQLException {
    ...
    try {
      ...
      throw new KSQLException("Failed to re-encode xml data.", KSQLState.DATA_ERROR, ioe);
    }
  }
  ...
  public synchronized <T extends Source> T getSource(Class<T> sourceClass) throws SQLException {
    ...
    try {
      ...
      throw new KSQLException(GT.tr("Unable to decode xml data."), KSQLState.DATA_ERROR, e);
    }

    throw new KSQLException(GT.tr("Unknown XML Source class: {0}", sourceClass),
  ...
  public synchronized <T extends Result> T setResult(Class<T> resultClass) throws SQLException {
    ...
    if (resultClass == null || DOMResult.class.equals(resultClass)) {
      ...
      try {
        ...
        throw new KSQLException(GT.tr("Unable to create SAXResult for SQLXML."),
            KSQLState.UNEXPECTED_ERROR, te);
      }
      ...
      try {
        ...
        throw new KSQLException(GT.tr("Unable to create StAXResult for SQLXML"),
            KSQLState.UNEXPECTED_ERROR, xse);
      }
    }

    throw new KSQLException(GT.tr("Unknown XML Result class: {0}", resultClass),
        KSQLState.INVALID_PARAMETER_TYPE);
  }
  ...
  private void checkFreed() throws SQLException {
    if (freed) {
      throw new KSQLException(GT.tr("This SQLXML object has already been freed."),
  ...
  private void ensureInitialized() throws SQLException {
    ...
    if (byteArrayOutputStream != null) {
      try {
        ...
        throw new KSQLException(GT.tr("Failed to convert binary xml data to encoding: {0}.",
            conn.getEncoding().name()), KSQLState.DATA_ERROR, ioe);
      } finally {
      ...
      try {
        ...
        throw new KSQLException(GT.tr("Unable to convert DOMResult SQLXML data to a string."),
            KSQLState.DATA_ERROR, te);
      } finally {
        domResult = null;
        active = false;
      }
