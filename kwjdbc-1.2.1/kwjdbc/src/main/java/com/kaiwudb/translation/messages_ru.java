...
public class messages_ru extends java.util.ResourceBundle {
  ...
  static {
    ...
    t[53] = "Невозможно обработать KWInterval: {0}";
    ...
    t[373] = "Не в состоянии ассоциировать значения параметров для команды (KWBindException)";
    ...
    t[528] = "The authentication type {0} is not supported. Check that you have configured the kw_hba.conf file to include the client''s IP address or subnet, and that it is using an authentication scheme supported by the driver.";
    t[529] = "Тип аутентификации {0} не поддерживается. Проверьте если вы сконфигурировали файл kw_hba.conf чтобы включить IP-адреса клиентов или подсеть. Также удостовертесь что он использует схему аутентификации поддерживаемую драйвером.";
