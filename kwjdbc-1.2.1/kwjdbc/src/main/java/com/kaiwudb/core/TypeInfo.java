...
public interface TypeInfo {
  void addCoreType(String kwTypeName, Integer oid, Integer sqlType, String javaClass,
      Integer arrayOid);
  ...
  /**
   * Look up the SQL typecode for a given kaiwudb type name.
   *
   * @param kwTypeName the server type name to look up
   * @return the SQL type code (a constant from {@link java.sql.Types}) for the type
   * @throws SQLException if an error occurs when retrieving sql type
   */
  int getSQLType(String kwTypeName) throws SQLException;

  /**
   * Look up the oid for a given kaiwudb type name. This is the inverse of
   * {@link #getKWType(int)}.
   *
   * @param kwTypeName the server type name to look up
   * @return the type's OID, or 0 if unknown
   * @throws SQLException if an error occurs when retrieving KW type
   */
  int getKWType(String kwTypeName) throws SQLException;

  /**
   * Look up the kaiwudb type name for a given oid. This is the inverse of
   * {@link #getKWType(String)}.
   *
   * @param oid the type's OID
   * @return the server type name for that OID or null if unknown
   * @throws SQLException if an error occurs when retrieving KW type
   */
  String getKWType(int oid) throws SQLException;
   ...
   */
  int getKWArrayElement(int oid) throws SQLException;

  /**
   * Determine the oid of the given base kaiwudb type's array type.
   *
   ...
   */
  int getKWArrayType(String elementTypeName) throws SQLException;
  ...
  Iterator<String> getKWTypeNamesWithSQLTypes();

  Class<? extends KWobject> getKWobject(String type);
