...
import com.kaiwudb.copy.CopyOut;
import com.kaiwudb.core.CommandCompleteParser;
import com.kaiwudb.core.Encoding;
import com.kaiwudb.core.EncodingPredictor;
import com.kaiwudb.core.Field;
import com.kaiwudb.core.NativeQuery;
import com.kaiwudb.core.Notification;
import com.kaiwudb.core.KWBindException;
import com.kaiwudb.core.ParameterList;
import com.kaiwudb.core.Parser;
import com.kaiwudb.core.Query;
import com.kaiwudb.core.QueryExecutor;
import com.kaiwudb.core.QueryExecutorBase;
import com.kaiwudb.core.ReplicationProtocol;
import com.kaiwudb.core.ResultCursor;
import com.kaiwudb.core.ResultHandler;
import com.kaiwudb.core.ResultHandlerBase;
import com.kaiwudb.core.ResultHandlerDelegate;
import com.kaiwudb.core.SqlCommandType;
import com.kaiwudb.core.Utils;
import com.kaiwudb.core.v3.replication.V3ReplicationProtocol;
import com.kaiwudb.jdbc.AutoSave;
import com.kaiwudb.jdbc.BatchResultHandler;
import com.kaiwudb.jdbc.PreferQueryMode;
import com.kaiwudb.jdbc.TimestampUtils;
import com.kaiwudb.util.GT;
import com.kaiwudb.util.KSQLState;
import com.kaiwudb.util.KSQLWarning;
import com.kaiwudb.util.ServerErrorMessage;
import com.kaiwudb.core.Oid;
import com.kaiwudb.core.KWStream;
import com.kaiwudb.core.SqlCommand;
import com.kaiwudb.core.TransactionState;
...
public class QueryExecutorImpl extends QueryExecutorBase {
  ...
  public QueryExecutorImpl(KWStream kwStream, String user, String database,
                           int cancelSignalTimeout, Properties info) throws SQLException, IOException {
    super(kwStream, user, database, cancelSignalTimeout, info);

    this.allowEncodingChanges = KWProperty.ALLOW_ENCODING_CHANGES.getBoolean(info);
    this.cleanupSavePoints = KWProperty.CLEANUP_SAVEPOINTS.getBoolean(info);
    this.replicationProtocol = new V3ReplicationProtocol(this, kwStream);
   ...
   * @throws KSQLException when already holding the lock or getting interrupted.
   */
  private void lock(Object obtainer) throws KSQLException {
    if (lockedFor == obtainer) {
      throw new KSQLException(GT.tr("Tried to obtain lock while already holding it"),
          KSQLState.OBJECT_NOT_IN_STATE);
   ...
   * @throws KSQLException when this thread does not hold the lock
   */
  private void unlock(Object holder) throws KSQLException {
    if (lockedFor != holder) {
      throw new KSQLException(GT.tr("Tried to break lock on database connection"),
          KSQLState.OBJECT_NOT_IN_STATE);
    }
    lockedFor = null;
    this.notify();
   ...
   */
  private void waitOnLock() throws KSQLException {
    while (lockedFor != null) {
      try {
        ...
        throw new KSQLException(
            ...
            KSQLState.OBJECT_NOT_IN_STATE, ie);
      }
    }
  }
  ...
  public synchronized void execute(Query query, ParameterList parameters, ResultHandler handler,
    ...
    try {
      try {
        ...
        estimatedReceiveBufferBytes = 0;
        handler
            .handleError(new KSQLException(GT.tr("Unable to bind parameter values for statement."),
                KSQLState.INVALID_PARAMETER_VALUE, se.getIOException()));
      }
    } catch (IOException e) {
      abort();
      handler.handleError(
          new KSQLException(GT.tr("An I/O error occurred while sending to the backend."),
    ...
    if (((flags & QueryExecutor.QUERY_SUPPRESS_BEGIN) == 0
      ...
      sendOneQuery(autoSaveQuery, SimpleQuery.NO_PARAMETERS, 1, 0,
          QUERY_NO_RESULTS | QUERY_NO_METADATA
              // KaiwuDB does not support bind, exec, simple, sync message flow,
  ...
  private void releaseSavePoint(boolean autosave, int flags) throws SQLException {
    if ( autosave
      ...
      try {
        ...
        throw  new KSQLException(GT.tr("Error releasing savepoint"), KSQLState.IO_ERROR);
      }
    }
  }
      ...
      public void handleCommandStatus(String status, long updateCount, long insertOID) {
        if (!sawBegin) {
          ...
          if (!status.equals("BEGIN")) {
            handleError(new KSQLException(GT.tr("Expected command status BEGIN, got {0}.", status),
  ...
  public synchronized byte[] fastpathCall(int fnid, ParameterList parameters, boolean suppressBegin)
    ...
    try {
      ...
      abort();
      throw new KSQLException(GT.tr("An I/O error occurred while sending to the backend."),
          KSQLState.CONNECTION_FAILURE, ioe);
    }
  }
      ...
      ResultHandler handler = new ResultHandlerBase() {
        ...
        public void handleCommandStatus(String status, long updateCount, long insertOID) {
          if (!sawBegin) {
            if (!status.equals("BEGIN")) {
              handleError(
                  new KSQLException(GT.tr("Expected command status BEGIN, got {0}.", status),
                      KSQLState.PROTOCOL_VIOLATION));
            }
            sawBegin = true;
          } else {
            handleError(new KSQLException(GT.tr("Unexpected command status: {0}.", status),
                KSQLState.PROTOCOL_VIOLATION));
          }
        }
  ...
  private void sendFastpathCall(int fnid, SimpleParameterList params)
    ...
    kwStream.sendChar('F');
    kwStream.sendInteger4(4 + 4 + 2 + 2 * paramCount + 2 + encodedSize + 2);
    kwStream.sendInteger4(fnid);
    kwStream.sendInteger2(paramCount);
    for (int i = 1; i <= paramCount; ++i) {
      kwStream.sendInteger2(params.isBinary(i) ? 1 : 0);
    }
    kwStream.sendInteger2(paramCount);
    for (int i = 1; i <= paramCount; i++) {
      if (params.isNull(i)) {
        kwStream.sendInteger4(-1);
      } else {
        kwStream.sendInteger4(params.getV3Length(i)); // Parameter size
        params.writeV3Value(i, kwStream);
      }
    }
    kwStream.sendInteger2(1); // Binary result format
    kwStream.flush();
  }
  ...
  public synchronized void processNotifies(int timeoutMillis) throws SQLException {
    ...
    if (useTimeout) {
      ...
      try {
        oldTimeout = kwStream.getSocket().getSoTimeout();
      } catch (SocketException e) {
        throw new KSQLException(GT.tr("An error occurred while trying to get the socket "
          + "timeout."), KSQLState.CONNECTION_FAILURE, e);
      }
    }

    try {
      while (timeoutMillis >= 0 || kwStream.hasMessagePending()) {
        ...
        switch (c) {
          ...
          case 'N': // Notice Response (warnings / info)
            ...
            break;
          default:
            throw new KSQLException(GT.tr("Unknown Response Type {0}.", (char) c),
  ...
  private void setSocketTimeout(int millis) throws KSQLException {
    try {
      Socket s = kwStream.getSocket();
      if (!s.isClosed()) { // Is this check required?
        kwStream.getSocket().setSoTimeout(millis);
      }
    } catch (SocketException e) {
      throw new KSQLException(GT.tr("An error occurred while trying to reset the socket timeout."),
        KSQLState.CONNECTION_FAILURE, e);
    }
  }
  ...
  public synchronized CopyOperation startCopy(String sql, boolean suppressBegin)
    ...
    try {
      ...
      kwStream.sendChar('Q');
      kwStream.sendInteger4(buf.length + 4 + 1);
      kwStream.send(buf);
      kwStream.sendChar(0);
      kwStream.flush();
      ...
      throw new KSQLException(GT.tr("Database connection failed when starting copy"),
          KSQLState.CONNECTION_FAILURE, ioe);
    }
  }
  ...
  private synchronized void initCopy(CopyOperationImpl op) throws SQLException, IOException {
    kwStream.receiveInteger4(); // length not used
    int rowFormat = kwStream.receiveChar();
    int numFields = kwStream.receiveInteger2();
    int[] fieldFormats = new int[numFields];

    for (int i = 0; i < numFields; i++) {
      fieldFormats[i] = kwStream.receiveInteger2();
    }
  ...
  public void cancelCopy(CopyOperationImpl op) throws SQLException {
    if (!hasLock(op)) {
      throw new KSQLException(GT.tr("Tried to cancel an inactive copy operation"),
          KSQLState.OBJECT_NOT_IN_STATE);
    }
    ...
    try {
      if (op instanceof CopyIn) {
        synchronized (this) {
          ...
          kwStream.sendChar('f'); // CopyFail
          kwStream.sendInteger4(5 + msg.length);
          kwStream.send(msg);
          kwStream.sendChar(0);
          kwStream.flush();
          do {
      ...
      throw new KSQLException(GT.tr("Database connection failed when canceling copy operation"),
          KSQLState.CONNECTION_FAILURE, ioe);
    } finally {
    ...
    if (op instanceof CopyIn) {
      if (errors < 1) {
        throw new KSQLException(GT.tr("Missing expected error response to copy cancel request"),
            KSQLState.COMMUNICATION_ERROR);
      } else if (errors > 1) {
        throw new KSQLException(
            GT.tr("Got {0} error responses to single copy cancel request", String.valueOf(errors)),
            KSQLState.COMMUNICATION_ERROR, error);
      }
    }
  }
  ...
  public synchronized long endCopy(CopyOperationImpl op) throws SQLException {
    if (!hasLock(op)) {
      throw new KSQLException(GT.tr("Tried to end inactive copy"), KSQLState.OBJECT_NOT_IN_STATE);
    }

    try {
      LOGGER.log(Level.FINEST, " FE=> CopyDone");

      kwStream.sendChar('c'); // CopyDone
      kwStream.sendInteger4(4);
      kwStream.flush();
      ...
      throw new KSQLException(GT.tr("Database connection failed when ending copy"),
          KSQLState.CONNECTION_FAILURE, ioe);
    }
  }
  ...
  public synchronized void writeToCopy(CopyOperationImpl op, byte[] data, int off, int siz)
      throws SQLException {
    if (!hasLock(op)) {
      throw new KSQLException(GT.tr("Tried to write to an inactive copy operation"),
          KSQLState.OBJECT_NOT_IN_STATE);
    }
    ...
    try {
      kwStream.sendChar('d');
      kwStream.sendInteger4(siz + 4);
      kwStream.send(data, off, siz);

    } catch (IOException ioe) {
      throw new KSQLException(GT.tr("Database connection failed when writing to copy"),
          KSQLState.CONNECTION_FAILURE, ioe);
    }
  }
  ...
  synchronized void readFromCopy(CopyOperationImpl op, boolean block) throws SQLException {
    if (!hasLock(op)) {
      throw new KSQLException(GT.tr("Tried to read from inactive copy"),
          KSQLState.OBJECT_NOT_IN_STATE);
    }
    ...
    try {
      ...
      throw new KSQLException(GT.tr("Database connection failed when reading from copy"),
          KSQLState.CONNECTION_FAILURE, ioe);
    }
  }
   ...
   * on kwStream or QueryExecutor are not allowed in a method after calling this!
   *
   * @param block whether to block waiting for input
  ...
  CopyOperationImpl processCopyResults(CopyOperationImpl op, boolean block)
    ...
    *  This is a hack as we should not end up here, but sometimes do with large copy operations.
    ...
    try {
      ...
      while (!endReceiving && (block || kwStream.hasMessagePending())) {
        ...
        switch (c) {
          ...
          case 'G': // CopyInResponse
            ...
            if (op != null) {
              error = new KSQLException(GT.tr("Got CopyInResponse from server during an active {0}",
                  op.getClass().getName()), KSQLState.OBJECT_NOT_IN_STATE);
            }
          ...
          case 'H': // CopyOutResponse
            ...
            if (op != null) {
              error = new KSQLException(GT.tr("Got CopyOutResponse from server during an active {0}",
                  op.getClass().getName()), KSQLState.OBJECT_NOT_IN_STATE);
            }
          ...
          case 'W': // CopyBothResponse
            ...
            if (op != null) {
              error = new KSQLException(GT.tr("Got CopyBothResponse from server during an active {0}",
                  op.getClass().getName()), KSQLState.OBJECT_NOT_IN_STATE);
            }
          ...
          case 'd': // CopyData
            ...
            len = kwStream.receiveInteger4() - 4;

            assert len > 0 : "Copy Data length must be greater than 4";

            byte[] buf = kwStream.receive(len);
            if (op == null) {
              error = new KSQLException(GT.tr("Got CopyData without an active copy operation"),
                  KSQLState.OBJECT_NOT_IN_STATE);
            } else if (!(op instanceof CopyOut)) {
              error = new KSQLException(
          ...
          case 'c': // CopyDone (expected after all copydata received)
            ...
            len = kwStream.receiveInteger4() - 4;
            if (len > 0) {
              kwStream.receive(len); // not in specification; should never appear
            }

            if (!(op instanceof CopyOut)) {
              error = new KSQLException("Got CopyDone while not copying from server",
                  KSQLState.OBJECT_NOT_IN_STATE);
            }
  ...
  private void flushIfDeadlockRisk(Query query, boolean disallowBatching,
    ...
    if (sq.isStatementDescribed()) {
       ...
       * not deadlock-proof though; see kwjdbc github issues #194 and #195.
       */
  ...
  private void sendQuery(Query query, V3ParameterList parameters, int maxRows, int fetchSize,
    ...
    // KwJDBC its self no longer does.
  ...
  private void sendSync() throws IOException {
    ...
    kwStream.sendChar('S'); // Sync
    kwStream.sendInteger4(4); // Length
    kwStream.flush();
  ...
  private void sendParse(SimpleQuery query, SimpleParameterList params, boolean oneShot)
    ...
    kwStream.sendChar('P'); // Parse
    kwStream.sendInteger4(encodedSize);
    if (encodedStatementName != null) {
      kwStream.send(encodedStatementName);
    }
    kwStream.sendChar(0); // End of statement name
    kwStream.send(queryUtf8); // Query string
    kwStream.sendChar(0); // End of query string.
    kwStream.sendInteger2(params.getParameterCount()); // # of parameter types specified
    for (int i = 1; i <= params.getParameterCount(); ++i) {
      kwStream.sendInteger4(params.getTypeOID(i));
    }
  ...
  private void sendBind(SimpleQuery query, SimpleParameterList params, Portal portal,
    ...
    if (encodedSize > 0x3fffffff) {
      throw new KWBindException(new IOException(GT.tr(
    ...
    kwStream.sendChar('B'); // Bind
    kwStream.sendInteger4((int) encodedSize); // Message size
    if (encodedPortalName != null) {
      kwStream.send(encodedPortalName); // Destination portal name.
    }
    kwStream.sendChar(0); // End of portal name.
    if (encodedStatementName != null) {
      kwStream.send(encodedStatementName); // Source statement name.
    }
    kwStream.sendChar(0); // End of statement name.

    kwStream.sendInteger2(params.getParameterCount()); // # of parameter format codes
    for (int i = 1; i <= params.getParameterCount(); ++i) {
      kwStream.sendInteger2(params.isBinary(i) ? 1 : 0); // Parameter format code
    }

    kwStream.sendInteger2(params.getParameterCount()); // # of parameter values
    ...
    // to the caller.
    //
    KWBindException bindException = null;

    for (int i = 1; i <= params.getParameterCount(); ++i) {
      if (params.isNull(i)) {
        kwStream.sendInteger4(-1); // Magic size of -1 means NULL
      } else {
        kwStream.sendInteger4(params.getV3Length(i)); // Parameter size
        try {
          params.writeV3Value(i, kwStream); // Parameter value
        } catch (KWBindException be) {
          bindException = be;
        }
      }
    }

    kwStream.sendInteger2(numBinaryFields); // # of result format codes
    for (int i = 0; i < numBinaryFields; ++i) {
      kwStream.sendInteger2(fields[i].getFormat());
    }
  ...
  private void sendDescribePortal(SimpleQuery query, Portal portal) throws IOException {
    ...
    kwStream.sendChar('D'); // Describe
    kwStream.sendInteger4(encodedSize); // message size
    kwStream.sendChar('P'); // Describe (Portal)
    if (encodedPortalName != null) {
      kwStream.send(encodedPortalName); // portal name to close
    }
    kwStream.sendChar(0); // end of portal name
  ...
  private void sendDescribeStatement(SimpleQuery query, SimpleParameterList params,
    ...
    kwStream.sendChar('D'); // Describe
    kwStream.sendInteger4(encodedSize); // Message size
    kwStream.sendChar('S'); // Describe (Statement);
    if (encodedStatementName != null) {
      kwStream.send(encodedStatementName); // Statement name
    }
    kwStream.sendChar(0); // end message
  ...
  private void sendExecute(SimpleQuery query, Portal portal, int limit) throws IOException {
    ...
    kwStream.sendChar('E'); // Execute
    kwStream.sendInteger4(4 + 1 + encodedSize + 4); // message size
    if (encodedPortalName != null) {
      kwStream.send(encodedPortalName); // portal name
    }
    kwStream.sendChar(0); // portal name terminator
    kwStream.sendInteger4(limit); // row limit
  ...
  private void sendClosePortal(String portalName) throws IOException {
    ...
    kwStream.sendChar('C'); // Close
    kwStream.sendInteger4(4 + 1 + 1 + encodedSize); // message size
    kwStream.sendChar('P'); // Close (Portal)
    if (encodedPortalName != null) {
      kwStream.send(encodedPortalName);
    }
    kwStream.sendChar(0); // unnamed portal
  }
  ...
  private void sendCloseStatement(String statementName) throws IOException {
    ...
    kwStream.sendChar('C'); // Close
    kwStream.sendInteger4(4 + 1 + encodedStatementName.length + 1); // message size
    kwStream.sendChar('S'); // Close (Statement)
    kwStream.send(encodedStatementName); // statement to close
    kwStream.sendChar(0); // statement name terminator
  }
  ...
  private void sendSimpleQuery(SimpleQuery query, SimpleParameterList params) throws IOException {
    ...
    Encoding encoding = kwStream.getEncoding();

    byte[] encoded = encoding.encode(nativeSql);
    kwStream.sendChar('Q');
    kwStream.sendInteger4(encoded.length + 4 + 1);
    kwStream.send(encoded);
    kwStream.sendChar(0);
    kwStream.flush();
    ...
    while (!endQuery) {
      ...
      switch (c) {
        ...
        case '1': // Parse Complete (response to Parse)
          kwStream.receiveInteger4(); // len, discarded
        ...
        case 't': { // ParameterDescription
          ...
          int numParams = kwStream.receiveInteger2();

          for (int i = 1; i <= numParams; i++) {
            int typeOid = kwStream.receiveInteger4();
        ...
        case 'D': // Data Transfer (ongoing Execute response)
          byte[][] tuple = null;
          try {
            tuple = kwStream.receiveTupleV3();
          } catch (OutOfMemoryError oome) {
            if (!noResults) {
              handler.handleError(
                  new KSQLException(GT.tr("Ran out of memory retrieving query results."),
                      KSQLState.OUT_OF_MEMORY, oome));
            }
          }
        ...
        case 'H': // CopyOutResponse
          ...
          handler.handleError(
              new KSQLException(GT.tr(COPY_ERROR_MESSAGE),
                  KSQLState.NOT_IMPLEMENTED));
          break;
  ...
  private Field[] receiveFields() throws IOException {
    kwStream.receiveInteger4(); // MESSAGE SIZE
    int size = kwStream.receiveInteger2();
    ...
    for (int i = 0; i < fields.length; i++) {
      String columnLabel = kwStream.receiveString();
      int tableOid = kwStream.receiveInteger4();
      short positionInTable = (short) kwStream.receiveInteger2();
      int typeOid = kwStream.receiveInteger4();
      int typeLength = kwStream.receiveInteger2();
      int typeModifier = kwStream.receiveInteger4();
      int formatType = kwStream.receiveInteger2();
  ...
  private void receiveAsyncNotify() throws IOException {
    int len = kwStream.receiveInteger4(); // MESSAGE SIZE
    assert len > 4 : "Length for AsyncNotify must be at least 4";

    int pid = kwStream.receiveInteger4();
    String msg = kwStream.receiveString();
    String param = kwStream.receiveString();
  ...
  private SQLException receiveErrorResponse() throws IOException {
    ...
    EncodingPredictor.DecodeResult totalMessage = kwStream.receiveErrorString(elen - 4);
    ...
    KSQLException error = new KSQLException(errorMsg, this.logServerErrorDetail);
  ...
  private SQLWarning receiveNoticeResponse() throws IOException {
    int nlen = kwStream.receiveInteger4();
    assert nlen > 4 : "Notice Response length must be greater than 4";

    ServerErrorMessage warnMsg = new ServerErrorMessage(kwStream.receiveString(nlen - 4));

    if (LOGGER.isLoggable(Level.FINEST)) {
      LOGGER.log(Level.FINEST, " <=BE NoticeResponse({0})", warnMsg.toString());
    }

    return new KSQLWarning(warnMsg);
  }
  ...
  private String receiveCommandStatus() throws IOException {
    ...
    String status = kwStream.receiveString(len - 5);
    // now read and discard the trailing \0
    kwStream.receiveChar(); // Receive(1) would allocate new byte[1], so avoid it
  ...
  private void receiveRFQ() throws IOException {
    if (kwStream.receiveInteger4() != 5) {
      throw new IOException("unexpected length of ReadyForQuery message");
    }

    char tStatus = (char) kwStream.receiveChar();
    if (LOGGER.isLoggable(Level.FINEST)) {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      switch (beresp) {
        ...
        case 'K':
          ...
          int pid = kwStream.receiveInteger4();
          int ckey = kwStream.receiveInteger4();
        ...
        case 'e':
          // receive analysis agent parameters, expand the KWDB connection parameters.
          receiveAgentParameter(info);
          break;
  ...
  public void receiveAgentParameter(Properties info) throws IOException {
    // receive analysis agent parameters, expand the KWDB connection parameters.
    kwStream.receiveInteger4(); // MESSAGE SIZE
    int paramCount = kwStream.receiveInteger4();
    for (int i = 0; i < paramCount; i++) {
      int keyLength = kwStream.receiveInteger4();
      // read key and intercept the terminator
      String key = kwStream.receiveString(keyLength + 1).substring(0, keyLength);
      kwStream.receiveInteger4(); // read oid,value data type
      int valueLength = kwStream.receiveInteger4();
      // read value and intercept the terminator
      String value = kwStream.receiveString(valueLength + 1).substring(0, valueLength);
  ...
  public void receiveParameterStatus() throws IOException, SQLException {
    // ParameterStatus
    kwStream.receiveInteger4(); // MESSAGE SIZE
    String name = kwStream.receiveString();
    String value = kwStream.receiveString();
      ...
      if (allowEncodingChanges) {
        if (!value.equalsIgnoreCase("UTF8") && !value.equalsIgnoreCase("UTF-8")) {
          LOGGER.log(Level.FINE,
              "kwjdbc expects client_encoding to be UTF8 for proper operation. Actual encoding is {0}",
              value);
        }
        kwStream.setEncoding(Encoding.getDatabaseEncoding(value));
        ...
        throw new KSQLException(GT.tr(
            ...
            value), KSQLState.CONNECTION_FAILURE);
  ...
  private final SimpleQuery autoSaveQuery =
      new SimpleQuery(
          new NativeQuery("SAVEPOINT KWJDBC_AUTOSAVE", new int[0], false, SqlCommand.BLANK),
          null, false);

  private final SimpleQuery releaseAutoSave =
      new SimpleQuery(
          new NativeQuery("RELEASE SAVEPOINT KWJDBC_AUTOSAVE", new int[0], false, SqlCommand.BLANK),
          null, false);
  ...
  private final SimpleQuery restoreToAutoSave =
      new SimpleQuery(
          new NativeQuery("ROLLBACK TO SAVEPOINT KWJDBC_AUTOSAVE", new int[0], false, SqlCommand.BLANK),
          null, false);
}
