...
public class V3KWReplicationStream implements KWReplicationStream {

  private static final Logger LOGGER = Logger.getLogger(V3KWReplicationStream.class.getName());
   ...
   *                         usually the currentLSN returned by calling kw_current_wal_lsn()for v10
   *                         above or kw_current_xlog_location() depending on the version of the
   *                         server
   ...
   */
  public V3KWReplicationStream(CopyDual copyDual, LogSequenceNumber startLSN, long updateIntervalMs,
  ...
  private ByteBuffer receiveNextData(boolean block) throws SQLException {
    ...
    } catch (KSQLException e) { //todo maybe replace on thread sleep?
  ...
  private void checkClose() throws KSQLException {
    if (isClosed()) {
      throw new KSQLException(GT.tr("This replication stream has been closed."),
