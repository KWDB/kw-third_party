...
public enum KWProperty {
   ...
   */
  KW_DBNAME("KWDBNAME", null,
      "Database name to connect to (may be specified directly in the JDBC URL)", true),

  /**
   * Hostname of the KaiwuDB server (may be specified directly in the JDBC URL).
   */
  KW_HOST("KWHOST", null,
      "Hostname of the KaiwuDB server (may be specified directly in the JDBC URL)", false),

  /**
   * Port of the KaiwuDB server (may be specified directly in the JDBC URL).
   */
  KW_PORT("KWPORT", null,
      "Port of the KaiwuDB server (may be specified directly in the JDBC URL)"),
...
//  /**
//   * Specify 'mode' connection initialization parameter.
//   * Expand the KWDB connection parameters.
//   */
//  KWDB_MODE("mode", null, "Specify 'mode' connection initialization parameter."),
//
//  /**
//   * Specify 'conn_id' connection initialization parameter.
//   * Expand the KWDB connection parameters.
//   */
//  KWDB_CONN_ID("conn_id", null, "Specify 'conn_id' connection initialization parameter."),
//
//  /**
//   * Specify time-series data engine url parameter to which you want to connect.
//   * Expand the KWDB connection parameters.
//   */
//  KWDB_TS_URL_EXPECT("ts_url_expect", null, "Specify time-series data engine url parameter to which you want to connect."),
   ...
   *
   * <p>Parameter should be use together with {@link KWProperty#LOGGER_LEVEL}</p>
   */
  ...
  /**
   * File containing the SSL Certificate. Default will be the file {@code kaiwudb.crt} in {@code
   * $HOME/.kaiwudb} (*nix) or {@code %APPDATA%\kaiwudb} (windows).
   */
  SSL_CERT("sslcert", null, "The location of the client's SSL certificate"),

  /**
   * File containing the SSL Key. Default will be the file {@code kaiwudb.pk8} in {@code
   * $HOME/.kaiwudb} (*nix) or {@code %APPDATA%\kaiwudb} (windows).
   */
   ...
   * KWKRBSRVNAME environment variable.
   */
   ...
   * default, {@code KAIWUDB}, is almost always correct.
   */
   ...
   * <p>Parameter should be use together with {@link KWProperty#ASSUME_MIN_SERVER_VERSION} with
  ...
  KWProperty(String name, String defaultValue, String description) {
    this(name, defaultValue, description, false);
  }

  KWProperty(String name, String defaultValue, String description, boolean required) {
   ...
   * @param properties properties to take actual value from
   * @return evaluated value for this connection parameter converted to int
   * @throws KSQLException if it cannot be converted to int.
   */
  public int getInt(Properties properties) throws KSQLException {
    ...
    try {
      ...
      throw new KSQLException(GT.tr("{0} parameter value must be an integer but was: {1}",
          getName(), value), KSQLState.INVALID_PARAMETER_VALUE, nfe);
    }
  }
   ...
   * @throws KSQLException if unable to parse property as integer
   */
