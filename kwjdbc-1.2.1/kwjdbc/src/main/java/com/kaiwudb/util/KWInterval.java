...
/**
 * This implements a class that handles the KaiwuDB interval type.
 */
public class KWInterval extends KWobject implements Serializable, Cloneable {
   ...
   * @see #setValue(String)
   */
  public KWInterval(String value) throws SQLException {
    this();
    setValue(value);
  }
   ...
   */
  public KWInterval(int years, int months, int days, int hours, int minutes, double seconds) {
    this();
   ...
   * format, that KaiwuDB returns - not all input formats are supported (e.g. '1 yr 2 m 3 s').
   *
   * @param value String representated interval (e.g. '3 years 2 mons')
   * @throws SQLException Is thrown if the string representation has an unknown format
   */
  ...
  public void setValue(String value) throws SQLException {
    final boolean KaiwuDBFormat = !value.startsWith("@");
    if (value.startsWith("P")) {
      parseISO8601Format(value);
      return;
    }
    // Just a simple '0'
    if (!KaiwuDBFormat && value.length() == 3 && value.charAt(2) == '0') {
    ...
    try {
      ...
      throw new KSQLException(GT.tr("Conversion of interval failed"),
          KSQLState.NUMERIC_CONSTANT_OUT_OF_RANGE, e);
    }

    if (!KaiwuDBFormat && value.endsWith("ago")) {
   ...
   */
  public void add(KWInterval interval) {
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof KWInterval)) {
      return false;
    }

    final KWInterval kwi = (KWInterval) obj;

    return kwi.years == years
        && kwi.months == months
        && kwi.days == days
        && kwi.hours == hours
        && kwi.minutes == minutes
        && kwi.wholeSeconds == wholeSeconds
        && kwi.microSeconds == microSeconds;
  }
