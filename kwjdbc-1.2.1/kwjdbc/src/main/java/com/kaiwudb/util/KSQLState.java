...
public enum KSQLState {
  ...
  public static boolean isConnectionError(String psqlState) {
    return KSQLState.CONNECTION_UNABLE_TO_CONNECT.getState().equals(psqlState)
        || KSQLState.CONNECTION_DOES_NOT_EXIST.getState().equals(psqlState)
        || KSQLState.CONNECTION_REJECTED.getState().equals(psqlState)
        || KSQLState.CONNECTION_FAILURE.getState().equals(psqlState)
        || KSQLState.CONNECTION_FAILURE_DURING_TRANSACTION.getState().equals(psqlState);
  }
