...
public class ConnectionPoolTest extends BaseDataSourceTest {
  ...
  protected PooledConnection getPooledConnection() throws SQLException {
    initializeDataSource();
    // we need to recast to KWConnectionPool rather than
    ...
    PooledConnection c = ((KWConnectionPoolDataSource) bds).getPooledConnection();
    connections.add(c);
    return c;
  }
  ...
  public void testBackendIsClosed() throws Exception {
    try {
      ...
      Assume.assumeTrue("pg_terminate_backend requires KaiwuDB 8.4+",
  ...
  /**
   * Ensure that a statement created from a pool can be used like any other statement in regard to kw extensions.
   */
  @Test
  public void testStatementsProxyKWStatement() {
    try {
      PooledConnection pc = getPooledConnection();
      con = pc.getConnection();

      Statement s = con.createStatement();
      boolean b = ((KWStatement) s).isUseServerPrepare();

      PreparedStatement ps = con.prepareStatement("select 'x'");
      b = ((KWStatement) ps).isUseServerPrepare();

      CallableStatement cs = con.prepareCall("select 'x'");
      b = ((KWStatement) cs).isUseServerPrepare();
