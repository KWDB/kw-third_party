...
public class ResultSetMetaDataTest extends BaseTest4 {
  ...
  protected void updateProperties(Properties props) {
    ...
    if (databaseMetadataCacheFields != null) {
      KWProperty.DATABASE_METADATA_CACHE_FIELDS.set(props, databaseMetadataCacheFields);
    }
    if (databaseMetadataCacheFieldsMib != null) {
      KWProperty.DATABASE_METADATA_CACHE_FIELDS_MIB.set(props, databaseMetadataCacheFieldsMib);
    }
  }
  ...
  private void runStandardTests(ResultSetMetaData rsmd) throws SQLException {
    KWResultSetMetaData kwrsmd = (KWResultSetMetaData) rsmd;
    ...
    assertEquals("", kwrsmd.getBaseColumnName(4));
    assertEquals("b", kwrsmd.getBaseColumnName(5));
    ...
    assertEquals("public", kwrsmd.getBaseSchemaName(1));
    assertEquals("", kwrsmd.getBaseSchemaName(4));

    assertEquals("rsmd1", rsmd.getTableName(1));
    assertEquals("", rsmd.getTableName(4));
    assertEquals("rsmd1", kwrsmd.getBaseTableName(1));
    assertEquals("", kwrsmd.getBaseTableName(4));
  ...
  public void testCache() throws Exception {
...

    {
      ...
      ResultSet rs = pstmt.executeQuery();
      KWResultSetMetaData kwrsmd = (KWResultSetMetaData) rs.getMetaData();
      assertEquals("a", kwrsmd.getBaseColumnName(1));
...

    {
      ...
      assertEquals(isCacheDisabled ? "b" : "a", kwrsmd.getBaseColumnName(1));
      TestUtil.closeQuietly(rs);
      TestUtil.closeQuietly(pstmt);
    }
  }
