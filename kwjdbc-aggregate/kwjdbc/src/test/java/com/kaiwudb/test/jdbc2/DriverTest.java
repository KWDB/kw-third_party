...
import com.kaiwudb.util.NullOutputStream;
...
/*
 * Tests the dynamically created class Driver
 *
 */
public class DriverTest {

  @Test
  public void urlIsNotForKaiwuDB() throws SQLException {
  ...
  public void testAcceptsURL() throws Exception {
    ...
    verifyUrl(drv, "jdbc:kaiwudb:test", "localhost", "26257", "test");
    verifyUrl(drv, "jdbc:kaiwudb://localhost/test", "localhost", "26257", "test");
    verifyUrl(drv, "jdbc:kaiwudb://localhost:26257/test", "localhost", "26257", "test");
    verifyUrl(drv, "jdbc:kaiwudb://127.0.0.1/anydbname", "127.0.0.1", "26257", "anydbname");
    verifyUrl(drv, "jdbc:kaiwudb://127.0.0.1:26257/hidden", "127.0.0.1", "26257", "hidden");
    verifyUrl(drv, "jdbc:kaiwudb://[::1]:26257/db", "[::1]", "26257", "db");

    // Badly formatted url's
    assertFalse(drv.acceptsURL("jdbc:kaiwudb:test"));
    assertFalse(drv.acceptsURL("kaiwudb:test"));
    assertFalse(drv.acceptsURL("db"));
    assertFalse(drv.acceptsURL("jdbc:kaiwudb://localhost:26257a/test"));
    assertFalse(drv.acceptsURL("jdbc:kaiwudb://localhost:500000/test"));
    assertFalse(drv.acceptsURL("jdbc:kaiwudb://localhost:0/test"));
    assertFalse(drv.acceptsURL("jdbc:kaiwudb://localhost:-2/test"));

    // failover urls
    verifyUrl(drv, "jdbc:kaiwudb://localhost,127.0.0.1:26257/test", "localhost,127.0.0.1",
        "26257,26257", "test");
    verifyUrl(drv, "jdbc:kaiwudb://localhost:26258,127.0.0.1:26257/test", "localhost,127.0.0.1",
        "26258,26257", "test");
    verifyUrl(drv, "jdbc:kaiwudb://[::1],[::1]:26257/db", "[::1],[::1]", "26257,26257", "db");
    verifyUrl(drv, "jdbc:kaiwudb://[::1]:5740,127.0.0.1:26257/db", "[::1],127.0.0.1", "5740,26257",
        "db");
  }
  ...
  private void verifyUrl(Driver drv, String url, String hosts, String ports, String dbName)
    ...
    assertEquals(url, dbName, p.getProperty(KWProperty.KW_DBNAME.getName()));
    assertEquals(url, hosts, p.getProperty(KWProperty.KW_HOST.getName()));
    assertEquals(url, ports, p.getProperty(KWProperty.KW_PORT.getName()));
  }
  ...
  /**
   * Tests that kwjdbc performs connection failover if unable to connect to the first host in the
   * URL.
   *
  ...
  public void testConnectFailover() throws Exception {
    String url = "jdbc:kaiwudb://invalidhost.not.here," + TestUtil.getServer() + ":"
  ...
  public void testSetLogWriter() throws Exception {
    ...
    try {
      ...
      Logger logger = Logger.getLogger("com.kaiwudb");
