 ...
 * (GUC_REPORT) parameters via KWConnection.getParameterStatuses() and
 * KWConnection.getParameterStatus().</p>
 */
public class ParameterStatusTest extends BaseTest4 {
  ...
  public void expectedInitialParameters() throws Exception {
    ...
    Map<String,String> params = ((KWConnection)con).getParameterStatuses();

    // KwJDBC forces the following parameters
    ...
    // KwJDBC sets TimeZone via Java's TimeZone.getDefault()
    // Kw reports POSIX timezones which are negated, so:
  ...
  public void reportUpdatedParameters() throws Exception {
    ...
    stmt.executeUpdate("SET application_name = 'kwjdbc_ParameterStatusTest2';");
    stmt.close();

    // Parameter status should be reported before the ReadyForQuery so we will
    // have already processed it
    Assert.assertEquals("kwjdbc_ParameterStatusTest2", ((KWConnection)con).getParameterStatus("application_name"));
  ...
  private void transactionalParametersCommon() throws Exception {
    ...
    Assert.assertEquals("Driver Tests", ((KWConnection)con).getParameterStatus("application_name"));

    // KwJDBC begins an explicit txn here due to autocommit=off so the effect
    // should be lost on rollback but retained on commit per the docs.
    stmt.executeUpdate("SET application_name = 'kwjdbc_ParameterStatusTestTxn';");
    Assert.assertEquals("kwjdbc_ParameterStatusTestTxn", ((KWConnection)con).getParameterStatus("application_name"));

    // SET LOCAL is always txn scoped so the effect here will always be
    // unwound on txn end.
    stmt.executeUpdate("SET LOCAL application_name = 'kwjdbc_ParameterStatusTestLocal';");
    Assert.assertEquals("kwjdbc_ParameterStatusTestLocal", ((KWConnection)con).getParameterStatus("application_name"));
  ...
  public void parameterMapReadOnly() throws Exception {
    try {
      con = TestUtil.openDB();
      Map params = ((KWConnection)con).getParameterStatuses();
  ...
  public void parameterMapIsView() throws Exception {
    ...
    Assert.assertEquals("Driver Tests", params.get("application_name"));
    stmt.executeUpdate("SET application_name = 'kwjdbc_paramstatus_view';");
    Assert.assertEquals("kwjdbc_paramstatus_view", params.get("application_name"));
