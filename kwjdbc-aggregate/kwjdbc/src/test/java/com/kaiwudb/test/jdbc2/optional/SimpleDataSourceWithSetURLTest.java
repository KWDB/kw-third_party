...
import static com.kaiwudb.Driver.parseURL;
...
public class SimpleDataSourceWithSetURLTest extends BaseDataSourceTest {
  ...
  protected void initializeDataSource() {
    if (bds == null) {
      ...
      bds.setURL(String.format("jdbc:kaiwudb://%s:%d/%s?prepareThreshold=%d&loggerLevel=%s", TestUtil.getServer(), TestUtil.getPort(), TestUtil.getDatabase(), TestUtil.getPrepareThreshold(),
  ...
  public void testGetURL() throws Exception {
    ...
    assertEquals(TestUtil.getServer(), properties.getProperty(KWProperty.KW_HOST.getName()));
    assertEquals(Integer.toString(TestUtil.getPort()), properties.getProperty(KWProperty.KW_PORT.getName()));
    assertEquals(TestUtil.getDatabase(), properties.getProperty(KWProperty.KW_DBNAME.getName()));
    assertEquals(Integer.toString(TestUtil.getPrepareThreshold()), properties.getProperty(KWProperty.PREPARE_THRESHOLD.getName()));
    assertEquals(TestUtil.getLogLevel(), properties.getProperty(KWProperty.LOGGER_LEVEL.getName()));
  }
