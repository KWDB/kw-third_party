...
/*
 * TestCase to test the internal functionality of com.kaiwudb.jdbc2.DatabaseMetaData's various
...
public class DatabaseMetaDataPropertiesTest {
  ...
  public void testCapabilities() throws SQLException {
    ...
    // This should always be false for kaiwudb (at least for 7.x)
  ...
  public void testDbProductDetails() throws SQLException {
    assertTrue(con instanceof KWConnection);

    DatabaseMetaData dbmd = con.getMetaData();
    assertNotNull(dbmd);

    assertEquals("KaiwuDB", dbmd.getDatabaseProductName());
  ...
  public void testDriverVersioning() throws SQLException {
    ...
    assertEquals("KaiwuDB JDBC Driver", dbmd.getDriverName());
    Assert.assertEquals(DriverInfo.DRIVER_VERSION, dbmd.getDriverVersion());
    Assert.assertEquals(new Driver().getMajorVersion(), dbmd.getDriverMajorVersion());
    Assert.assertEquals(new Driver().getMinorVersion(), dbmd.getDriverMinorVersion());
    assertTrue(dbmd.getJDBCMajorVersion() >= 0);
