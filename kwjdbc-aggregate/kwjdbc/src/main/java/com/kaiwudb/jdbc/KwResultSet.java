...
import com.kaiwudb.KWRefCursorResultSet;
...
import com.kaiwudb.util.HStoreConverter;
import com.kaiwudb.util.KSQLException;
import com.kaiwudb.util.KSQLState;
import com.kaiwudb.util.KWbytea;
...
public class KwResultSet implements ResultSet, KWRefCursorResultSet {
  ...
  protected ResultSetMetaData createMetaData() throws SQLException {
    return new KwResultSetMetaData(connection, fields);
  }
  ...
  KwResultSet(Query originalQuery, BaseStatement statement, Field[] fields, List<byte[][]> tuples,
  ...
  protected Object internalGetObject(int columnIndex, Field field) throws SQLException {
    switch (getSQLType(columnIndex)) {
      ...
      default:
        String type = getKWType(columnIndex);
        ...
        if (type.equals("refcursor")) {
          ...
          ((KwResultSet) rs).setRefCursor(cursorName);
          return rs;
        }
  ...
  private void checkScrollable() throws SQLException {
    ...
    if (resultsettype == ResultSet.TYPE_FORWARD_ONLY) {
      throw new KSQLException(
        ...
        KSQLState.INVALID_CURSOR_STATE);
    }
  }
  ...
  protected Array makeArray(int oid, byte[] value) throws SQLException {
    return new KwArray(connection, oid, value);
  }
  ...
  protected Blob makeBlob(long oid) throws SQLException {
    return new KwBlob(connection, oid);
  }
  ...
  public java.io.Reader getCharacterStream(int i) throws SQLException {
    ...
    // Version 7.2 supports AsciiStream for all the KW text types
    // As the spec/javadoc for this method indicate this is to be used for
    // large text values (i.e. LONGVARCHAR) KW doesn't have a separate
  ...
  protected Clob makeClob(long oid) throws SQLException {
    return new KwClob(connection, oid);
  }
  ...
  public boolean previous() throws SQLException {
    ...
    if (onInsertRow) {
      throw new KSQLException(GT.tr("Can''t use relative move methods while on the insert row."),
        KSQLState.INVALID_CURSOR_STATE);
    }
  ...
  public void setFetchDirection(int direction) throws SQLException {
    ...
    switch (direction) {
      ...
      case ResultSet.FETCH_UNKNOWN:
        ...
        break;
      default:
        throw new KSQLException(GT.tr("Invalid fetch direction constant: {0}.", direction),
  ...
  public synchronized void cancelRowUpdates() throws SQLException {
    checkClosed();
    if (onInsertRow) {
      throw new KSQLException(GT.tr("Cannot call cancelRowUpdates() when on the insert row."),
        KSQLState.INVALID_CURSOR_STATE);
    }
  ...
  public synchronized void deleteRow() throws SQLException {
    ...
    if (onInsertRow) {
      throw new KSQLException(GT.tr("Cannot call deleteRow() when on the insert row."),
        KSQLState.INVALID_CURSOR_STATE);
    }
    ...
    if (rows.isEmpty()) {
      throw new KSQLException(GT.tr("There are no rows in this ResultSet."),
        KSQLState.INVALID_CURSOR_STATE);
    }
  ...
  public synchronized void insertRow() throws SQLException {
    ...
    if (!onInsertRow) {
      throw new KSQLException(GT.tr("Not on the insert row."), KSQLState.INVALID_CURSOR_STATE);
    } else if (updateValues.isEmpty()) {
      throw new KSQLException(GT.tr("You must specify at least one column value to insert a row."),
        KSQLState.INVALID_PARAMETER_VALUE);
    } else {
      ...
      if (usingOID) {
        ...
        long insertedOID = ((KwStatement) insertStatement).getLastOID();
  ...
  public synchronized void updateAsciiStream(int columnIndex, java.io.InputStream x, int length)
    ...
    try {
      ...
      throw new KSQLException(GT.tr("The JVM claims not to support the encoding: {0}", "ASCII"),
        KSQLState.UNEXPECTED_ERROR, uee);
    } catch (IOException ie) {
      throw new KSQLException(GT.tr("Provided InputStream failed."), null, ie);
    }
  }
  ...
  public synchronized void updateCharacterStream(int columnIndex, java.io.Reader x, int length)
    ...
    try {
      ...
      throw new KSQLException(GT.tr("Provided Reader failed."), null, ie);
    }
  }
  ...
  public synchronized void updateNull(int columnIndex) throws SQLException {
    ...
    String columnTypeName = getKWType(columnIndex);
  ...
  public void refreshRow() throws SQLException {
    checkUpdateable();
    if (onInsertRow) {
      throw new KSQLException(GT.tr("Can''t refresh the insert row."),
        KSQLState.INVALID_CURSOR_STATE);
    }
    ...
    KWResultSetMetaData kwmd = (KWResultSetMetaData) rsmd;
    for (int i = 1; i <= rsmd.getColumnCount(); i++) {
      if (i > 1) {
        selectSQL.append(", ");
      }
      selectSQL.append(kwmd.getBaseColumnName(i));
    }
    ...
    KwResultSet rs = (KwResultSet) selectStatement.executeQuery();
  ...
  public synchronized void updateRow() throws SQLException {
    ...
    if (onInsertRow) {
      throw new KSQLException(GT.tr("Cannot call updateRow() when on the insert row."),
        KSQLState.INVALID_CURSOR_STATE);
    }
  ...
  boolean isUpdateable() throws SQLException {
    ...
    if (i < 1) {
      throw new KSQLException(GT.tr("No primary key found for table {0}.", tableName),
        KSQLState.DATA_ERROR);
    }
  ...
  private void updateRowBuffer() throws SQLException {
    ...
    for (Map.Entry<String, Object> entry : updateValues.entrySet()) {
      ...
      if (valueObject instanceof KWobject) {
        String value = ((KWobject) valueObject).getValue();
            ...
            if (isBinary(columnIndex + 1)) {
              ...
              try {
                rowBuffer[columnIndex] =
                  KWbytea.toKWString((byte[]) valueObject).getBytes("ISO-8859-1");
              } catch (UnsupportedEncodingException e) {
                throw new KSQLException(
                  GT.tr("The JVM claims not to support the encoding: {0}", "ISO-8859-1"),
                  KSQLState.UNEXPECTED_ERROR, e);
              }
            }
            break;
  ...
  public class CursorResultHandler extends ResultHandlerBase {
    ...
    public void handleResultRows(Query fromQuery, Field[] fields, List<byte[][]> tuples,
                                 ResultCursor cursor) {
      KwResultSet.this.rows = tuples;
      KwResultSet.this.cursor = cursor;
    }
    ...
    public void handleCompletion() throws SQLException {
      ...
      if (warning != null) {
        KwResultSet.this.addWarning(warning);
      }
      super.handleCompletion();
    }
  }

  public BaseStatement getKWStatement() {
    return statement;
  }

  //
  // Backwards compatibility with KWRefCursorResultSet
  //
  ...
  public String getRefCursor() {
    // Can't check this because the KWRefCursorResultSet
  ...
  public void close() throws SQLException {
    try {
      ...
      ((KwStatement) statement).checkCompletion();
    }
  }

  /*
  used by KwStatement.closeForNextExecution to avoid
  ...
  public String getString(int columnIndex) throws SQLException {
    ...
    if (isBinary(columnIndex) && getSQLType(columnIndex) != Types.VARCHAR) {
      ...
      if (obj == null) {
        ...
        // KWobject based types like geometric types but getObject does
      ...
      if ("hstore".equals(getKWType(columnIndex))) {
    ...
    if (s != null) {
      ...
      try {
        ...
        try {
          ...
          if (gt > 0 || lt < 0) {
            throw new KSQLException(GT.tr("Bad value for type {0} : {1}", "byte", s),
              KSQLState.NUMERIC_VALUE_OUT_OF_RANGE);
          }
          return i.byteValue();
  ...
  public InputStream getAsciiStream(int columnIndex) throws SQLException {
    ...
    try {
      ...
      throw new KSQLException(GT.tr("The JVM claims not to support the encoding: {0}", "ASCII"),
        KSQLState.UNEXPECTED_ERROR, l_uee);
    }
  }
  ...
  public InputStream getUnicodeStream(int columnIndex) throws SQLException {
    ...
    try {
      ...
      throw new KSQLException(GT.tr("The JVM claims not to support the encoding: {0}", "UTF-8"),
        KSQLState.UNEXPECTED_ERROR, l_uee);
    }
  }
  ...
  public InputStream getBinaryStream(int columnIndex) throws SQLException {
    ...
    // Version 7.2 supports BinaryStream for all KW bytea type
    // As the spec/javadoc for this method indicate this is to be used for
    // large binary values (i.e. LONGVARBINARY) KW doesn't have a separate
  ...
  public Object getObject(int columnIndex) throws SQLException {
    ...
    if (isBinary(columnIndex)) {
      return connection.getObject(getKWType(columnIndex), null, thisRow[columnIndex - 1]);
    }
    return connection.getObject(getKWType(columnIndex), getString(columnIndex), null);
  }
  ...
  private String trimMoney(String s) {
    ...
    if (ch == '(') {
      s = "-" + KWtokenizer.removePara(s).substring(1);
    } else if (ch == '$') {
      s = s.substring(1);
  ...
  protected String getKWType(int column) throws SQLException {
    Field field = fields[column - 1];
    initSqlType(field);
    return field.getKWType();
  }
  ...
  private void initSqlType(Field field) throws SQLException {
    ...
    int oid = field.getOID();
    String kwType = typeInfo.getKWType(oid);
    int sqlType = typeInfo.getSQLType(kwType);
    field.setSQLType(sqlType);
    field.setKWType(kwType);
  }
  ...
  protected void checkClosed() throws SQLException {
    if (rows == null) {
      throw new KSQLException(GT.tr("This ResultSet is closed."), KSQLState.OBJECT_NOT_IN_STATE);
    }
  }
    ...
    if (s != null) {
      try {
        ...
        try {
          ...
          if (gt > 0 || lt < 0) {
            throw new KSQLException(GT.tr("Bad value for type {0} : {1}", "short", s),
              KSQLState.NUMERIC_VALUE_OUT_OF_RANGE);
          }
          return i.shortValue();
    ...
    if (s != null) {
      try {
        ...
        try {
          ...
          if (gt > 0 || lt < 0) {
            throw new KSQLException(GT.tr("Bad value for type {0} : {1}", "int", s),
              KSQLState.NUMERIC_VALUE_OUT_OF_RANGE);
          }
          return i.intValue();
    ...
    if (s != null) {
      try {
        ...
        try {
          ...
          if (gt > 0 || lt < 0) {
            throw new KSQLException(GT.tr("Bad value for type {0} : {1}", "long", s),
              KSQLState.NUMERIC_VALUE_OUT_OF_RANGE);
          }
          return i.longValue();
        } catch (NumberFormatException ne) {
  ...
  public static BigDecimal toBigDecimal(String s) throws SQLException {
    ...
    try {
      ...
      throw new KSQLException(GT.tr("Bad value for type {0} : {1}", "BigDecimal", s),
        KSQLState.NUMERIC_VALUE_OUT_OF_RANGE);
    }
  }
  ...
  public static float toFloat(String s) throws SQLException {
    if (s != null) {
      try {
        ...
        throw new KSQLException(GT.tr("Bad value for type {0} : {1}", "float", s),
          KSQLState.NUMERIC_VALUE_OUT_OF_RANGE);
      }
    }
    return 0; // SQL NULL
  ...
  public static double toDouble(String s) throws SQLException {
    if (s != null) {
      try {
        ...
        throw new KSQLException(GT.tr("Bad value for type {0} : {1}", "double", s),
          KSQLState.NUMERIC_VALUE_OUT_OF_RANGE);
      }
    }
    return 0; // SQL NULL
   ...
   * @throws KSQLException If the field type is not supported numeric type.
   */
  private double readDoubleValue(byte[] bytes, int oid, String targetType) throws KSQLException {
    ...
    throw new KSQLException(GT.tr("Cannot convert the column of type {0} to requested type {1}.",
      Oid.toString(oid), targetType), KSQLState.DATA_TYPE_MISMATCH);
  }
   ...
   * @throws KSQLException If the field type is not supported numeric type or if the value is out of
   *                       range.
   */
  private long readLongValue(byte[] bytes, int oid, long minVal, long maxVal, String targetType)
    ...
    if (val < minVal || val > maxVal) {
      throw new KSQLException(GT.tr("Bad value for type {0} : {1}", targetType, val),
        KSQLState.NUMERIC_VALUE_OUT_OF_RANGE);
    }
    return val;
  }
  ...
  protected void updateValue(int columnIndex, Object value) throws SQLException {
    ...
    if (value == null) {
      ...
      KWResultSetMetaData md = (KWResultSetMetaData) getMetaData();
  ...
  protected Object getUUID(String data) throws SQLException {
    ...
    try {
      ...
      throw new KSQLException(GT.tr("Invalid UUID data."), KSQLState.INVALID_PARAMETER_VALUE, iae);
    }
  ...
  // NullObject is a simple extension of KWobject that always returns null
  // values but retains column type info.
  //

  static class NullObject extends KWobject {
  ...
  public <T> T getObject(int columnIndex, Class<T> type) throws SQLException {
    ...
    if (type == BigDecimal.class) {
      if (sqlType == Types.NUMERIC || sqlType == Types.DECIMAL) {
        ...
        throw new KSQLException(GT.tr("conversion to {0} from {1} not supported", type, getKWType(columnIndex)),
          KSQLState.INVALID_PARAMETER_VALUE);
      }
      ...
      try {
        ...
        throw new KSQLException(GT.tr("Invalid Inet data."), KSQLState.INVALID_PARAMETER_VALUE, ex);
      }
      // JSR-310 support
      //#if mvn.project.property.postgresql.jdbc.spec >= "JDBC4.2"
      ...
      if (sqlType == Types.DATE) {
        ...
        if (time == KWStatement.DATE_POSITIVE_INFINITY) {
          return type.cast(LocalDate.MAX);
        }
        if (time == KWStatement.DATE_NEGATIVE_INFINITY) {
      ...
      //#endif
    } else if (KWobject.class.isAssignableFrom(type)) {
      Object object;
      if (isBinary(columnIndex)) {
        object = connection.getObject(getKWType(columnIndex), null, thisRow[columnIndex - 1]);
      } else {
        object = connection.getObject(getKWType(columnIndex), getString(columnIndex), null);
      }
  ...
  public SQLXML getSQLXML(int columnIndex) throws SQLException {
    ...
    return new KwSQLXML(connection, data);
  }
