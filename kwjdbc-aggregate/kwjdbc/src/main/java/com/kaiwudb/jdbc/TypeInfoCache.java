...
public class TypeInfoCache implements TypeInfo {
  ...
  private static final Object[][] types = {
      ...
      {"varbytea", Oid.VARBYTEA, Types.VARBINARY, "[B", Oid.VARBYTEA_ARRAY},
      ...
      //#endif
      {"json", Oid.JSON, Types.OTHER, "KWobject", Oid.JSON_ARRAY},
      {"point", Oid.POINT, Types.OTHER, "KWpoint", Oid.POINT_ARRAY}
  };

  /**
   * KW maps several alias to real type names. When we do queries against kw_catalog, we must use
  ...
  public TypeInfoCache(BaseConnection conn, int unknownLength) {
    ...
    oidToKwName = new HashMap<Integer, String>((int) Math.round(types.length * 1.5));
    kwNameToOid = new HashMap<String, Integer>((int) Math.round(types.length * 1.5));
    kwNameToJavaClass = new HashMap<String, String>((int) Math.round(types.length * 1.5));
    kwNameToKwObject = new HashMap<String, Class<? extends KWobject>>((int) Math.round(types.length * 1.5));
    kwArrayToKwType = new HashMap<Integer, Integer>((int) Math.round(types.length * 1.5));
    ...
    // from getKWTypeNamesWithSQLTypes()
    kwNameToSQLType = Collections.synchronizedMap(new HashMap<String, Integer>((int) Math.round(types.length * 1.5)));
    ...
    for (Object[] type : types) {
      ...
      addCoreType(kwTypeName, oid, sqlType, javaClass, arrayOid);
    }

    kwNameToJavaClass.put("hstore", Map.class.getName());
  }

  @Override
  public synchronized void addCoreType(String kwTypeName, Integer oid, Integer sqlType,
      String javaClass, Integer arrayOid) {
    kwNameToJavaClass.put(kwTypeName, javaClass);
    kwNameToOid.put(kwTypeName, oid);
    oidToKwName.put(oid, kwTypeName);
    kwArrayToKwType.put(arrayOid, oid);
    kwNameToSQLType.put(kwTypeName, sqlType);
    ...
    kwNameToJavaClass.put(kwArrayTypeName, "java.sql.Array");
    kwNameToSQLType.put(kwArrayTypeName, Types.ARRAY);
    kwNameToOid.put(kwArrayTypeName, arrayOid);
    kwArrayTypeName = "_" + kwTypeName;
    if (!kwNameToJavaClass.containsKey(kwArrayTypeName)) {
      kwNameToJavaClass.put(kwArrayTypeName, "java.sql.Array");
      kwNameToSQLType.put(kwArrayTypeName, Types.ARRAY);
      kwNameToOid.put(kwArrayTypeName, arrayOid);
      oidToKwName.put(arrayOid, kwArrayTypeName);
    }
  }

  @Override
  public synchronized void addDataType(String type, Class<? extends KWobject> klass)
      throws SQLException {
    kwNameToKwObject.put(type, klass);
    kwNameToJavaClass.put(type, klass.getName());
  }

  @Override
  public Iterator<String> getKWTypeNamesWithSQLTypes() {
    return kwNameToSQLType.keySet().iterator();
  }

  @Override
  public int getSQLType(int oid) throws SQLException {
    return getSQLType(getKWType(oid));
  }
  ...
  public synchronized int getSQLType(String kwTypeName) throws SQLException {
    ...
    Integer i = kwNameToSQLType.get(kwTypeName);
    if (i != null) {
      return i;
    }
    ...
    getTypeInfoStatement.setString(1, kwTypeName);
    ...
    kwNameToSQLType.put(kwTypeName, type);
    return type;
  }
  ...
  private PreparedStatement getOidStatement(String kwTypeName) throws SQLException {
    ...
    int dotIndex = kwTypeName.indexOf('.');
    ...
    String fullName = isArray ? kwTypeName.substring(0, kwTypeName.length() - 2) : kwTypeName;
    String schema;
    String name;
    // simple use case
    if (dotIndex == -1) {
  ...
  public synchronized int getKWType(String kwTypeName) throws SQLException {
    Integer oid = kwNameToOid.get(kwTypeName);
    if (oid != null) {
      return oid;
    }

    PreparedStatement oidStatement = getOidStatement(kwTypeName);
    ...
    if (rs.next()) {
      ...
      oidToKwName.put(oid, internalName);
      kwNameToOid.put(internalName, oid);
    }
    kwNameToOid.put(kwTypeName, oid);
    rs.close();
  ...
  public synchronized String getKWType(int oid) throws SQLException {
    ...
    String kwTypeName = oidToKwName.get(oid);
    if (kwTypeName != null) {
      return kwTypeName;
    }
    ...
    if (rs.next()) {
      ...
      if (onPath) {
        kwTypeName = name;
        kwNameToOid.put(schema + "." + name, oid);
      } else {
        // TODO: escaping !?
  ...
  public int getKWArrayType(String elementTypeName) throws SQLException {
    ...
    return getKWType(elementTypeName + "[]");
  }
  ...
  protected synchronized int convertArrayToBaseOid(int oid) {
    Integer i = kwArrayToKwType.get(oid);
    if (i == null) {
      return oid;
    }
    return i;
  ...
  public synchronized int getKWArrayElement(int oid) throws SQLException {
    ...
    Integer kwType = kwArrayToKwType.get(oid);
    ...
    kwArrayToKwType.put(oid, kwType);
    kwNameToOid.put(schema + "." + name, kwType);
    String fullName = "\"" + schema + "\".\"" + name + "\"";
    kwNameToOid.put(fullName, kwType);
    if (onPath && name.equals(name.toLowerCase())) {
      oidToKwName.put(kwType, name);
      kwNameToOid.put(name, kwType);
    } else {
      oidToKwName.put(kwType, fullName);
    }
  ...
  public synchronized Class<? extends KWobject> getKWobject(String type) {
    return kwNameToKwObject.get(type);
  }

  @Override
  public synchronized String getJavaClass(int oid) throws SQLException {
    String kwTypeName = getKWType(oid);

    String result = kwNameToJavaClass.get(kwTypeName);
    if (result != null) {
      return result;
    }

    if (getSQLType(kwTypeName) == Types.ARRAY) {
      result = "java.sql.Array";
      kwNameToJavaClass.put(kwTypeName, result);
    }
