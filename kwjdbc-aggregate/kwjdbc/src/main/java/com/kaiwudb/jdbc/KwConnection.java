...
import com.kaiwudb.fastpath.Fastpath;
...
import com.kaiwudb.largeobject.LargeObjectManager;
import com.kaiwudb.replication.KWReplicationConnection;
import com.kaiwudb.replication.KWReplicationConnectionImpl;
...
import com.kaiwudb.util.KWmoney;
...
public class KwConnection implements BaseConnection {

  private static final Logger LOGGER = Logger.getLogger(KwConnection.class.getName());
  ...
  /**
   * Replication protocol in current version kaiwudb(10devel) supports a limited number of
   * commands.
   */
  ...
  // Ctor.
  //
  public KwConnection(HostSpec[] hostSpecs,
                      String user,
                      String database,
                      Properties info,
    ...
    this.readOnlyBehavior = getReadOnlyBehavior(KWProperty.READ_ONLY_MODE.get(info));

    setDefaultFetchSize(KWProperty.DEFAULT_ROW_FETCH_SIZE.getInt(info));

    setPrepareThreshold(KWProperty.PREPARE_THRESHOLD.getInt(info));
    ...
    if (KWProperty.READ_ONLY.getBoolean(info)) {
      setReadOnly(true);
    }

    this.hideUnprivilegedObjects = KWProperty.HIDE_UNPRIVILEGED_OBJECTS.getBoolean(info);
    ...
    String stringType = KWProperty.STRING_TYPE.get(info);
    if (stringType != null) {
    ...
    int unknownLength = KWProperty.UNKNOWN_LENGTH.getInt(info);

    // Initialize object handling
    typeCache = createTypeInfo(this, unknownLength);
    initObjectTypes(info);

    if (KWProperty.LOG_UNCLOSED_CONNECTIONS.getBoolean(info)) {
      openStackTrace = new Throwable("Connection was created at this point:");
    }
    this.disableColumnSanitiser = KWProperty.DISABLE_COLUMN_SANITISER.getBoolean(info);
    ...
    fieldMetadataCache = new LruCache<FieldMetadata.Key, FieldMetadata>(
      Math.max(0, KWProperty.DATABASE_METADATA_CACHE_FIELDS.getInt(info)),
      Math.max(0, KWProperty.DATABASE_METADATA_CACHE_FIELDS_MIB.getInt(info) * 1024 * 1024),
      false);

    replicationConnection = KWProperty.REPLICATION.get(info) != null;
  }
  ...
  private static Set<Integer> getBinaryOids(Properties info) throws KSQLException {
    boolean binaryTransfer = KWProperty.BINARY_TRANSFER.getBoolean(info);
    ...
    binaryOids.addAll(getOidSet(KWProperty.BINARY_TRANSFER_ENABLE.get(info)));
    binaryOids.removeAll(getOidSet(KWProperty.BINARY_TRANSFER_DISABLE.get(info)));
  ...
  public ResultSet execSQLQuery(String s, int resultSetType, int resultSetConcurrency)
    ...
    if (!hasResultSet) {
      throw new KSQLException(GT.tr("No results were returned by the query."), KSQLState.NO_DATA);
    }
  ...
  public void execSQLUpdate(String s) throws SQLException {
    ...
    if (stmt.executeWithFlags(s, QueryExecutor.QUERY_NO_METADATA | QueryExecutor.QUERY_NO_RESULTS
      ...
      throw new KSQLException(GT.tr("A result was returned when none was expected."),
        KSQLState.TOO_MANY_RESULTS);
    }
  ...
  public Object getObject(String type, String value, byte[] byteValue) throws SQLException {
    if (typemap != null) {
      ...
      if (c != null) {
        ...
        throw new KSQLException(GT.tr("Custom type maps are not supported."),
    ...
    try {
      Class<? extends KWobject> klass = typeCache.getKWobject(type);

      // If className is not null, then try to instantiate it,
      // It must be basetype KWobject

      // This is used to implement the com.kaiwudb unique types (like lseg,
      // point, etc).

      if (klass != null) {
        obj = klass.newInstance();
        obj.setType(type);
        if (byteValue != null && obj instanceof KWBinaryObject) {
          KWBinaryObject binObj = (KWBinaryObject) obj;
        ...
        // so return a KWobject with the type set, and the value set
        obj = new KWobject();
        obj.setType(type);
        obj.setValue(value);
      }
      ...
      throw new KSQLException(GT.tr("Failed to create object for: {0}.", type),
  ...
  public void addDataType(String type, String name) {
    try {
      addDataType(type, Class.forName(name).asSubclass(KWobject.class));
    } catch (Exception e) {
  ...
  @Override
  public void addDataType(String type, Class<? extends KWobject> klass) throws SQLException {
    checkClosed();
  ...
  private void initObjectTypes(Properties info) throws SQLException {
    ...
    addDataType("box", KWbox.class);
    addDataType("circle", KWcircle.class);
    addDataType("line", KWline.class);
    addDataType("lseg", KWlseg.class);
    addDataType("path", KWpath.class);
    addDataType("point", KWpoint.class);
    addDataType("polygon", KWpolygon.class);
    addDataType("money", KWmoney.class);
    addDataType("interval", KWInterval.class);
        ...
        try {
          ...
          throw new KSQLException(
            GT.tr("Unable to load the class {0} responsible for the datatype {1}",
              className, typeName),
            KSQLState.SYSTEM_ERROR, cnfe);
        }

        addDataType(typeName, klass.asSubclass(KWobject.class));
      }
    }
  }
  ...
  public void setReadOnly(boolean readOnly) throws SQLException {
    ...
    if (queryExecutor.getTransactionState() != TransactionState.IDLE) {
      throw new KSQLException(
        ...
        KSQLState.ACTIVE_SQL_TRANSACTION);
    }
  ...
  public void commit() throws SQLException {
    ...
    if (autoCommit) {
      throw new KSQLException(GT.tr("Cannot commit when autoCommit is enabled."),
        KSQLState.NO_ACTIVE_SQL_TRANSACTION);
    }
  ...
  protected void checkClosed() throws SQLException {
    if (isClosed()) {
      throw new KSQLException(GT.tr("This connection has been closed."),
        KSQLState.CONNECTION_DOES_NOT_EXIST);
    }
  }
  ...
  public void rollback() throws SQLException {
    ...
    if (autoCommit) {
      throw new KSQLException(GT.tr("Cannot rollback when autoCommit is enabled."),
        KSQLState.NO_ACTIVE_SQL_TRANSACTION);
    }
  ...
  public void setTransactionIsolation(int level) throws SQLException {
    ...
    if (isolationLevelName == null) {
      throw new KSQLException(GT.tr("Transaction isolation level {0} not supported.", level),
        KSQLState.NOT_IMPLEMENTED);
    }
   ...
   * fortnight enough clients were open to kill the kaiwudb server.</p>
   */
  @Override
  ...
  public byte[] encodeString(String str) throws SQLException {
    try {
      ...
      throw new KSQLException(GT.tr("Unable to translate data into the desired encoding."),
        KSQLState.DATA_ERROR, ioe);
    }
  }
  ...
  @Override
  public KWNotification[] getNotifications() throws SQLException {
    return getNotifications(-1);
  }

  @Override
  public KWNotification[] getNotifications(int timeoutMillis) throws SQLException {
    checkClosed();
    getQueryExecutor().processNotifies(timeoutMillis);
    // Backwards-compatibility hand-holding.
    KWNotification[] notifications = queryExecutor.getNotifications();
  ...
  private class TransactionCommandHandler extends ResultHandlerBase {
    ...
    public void handleCompletion() throws SQLException {
      ...
      if (warning != null) {
        KwConnection.this.addWarning(warning);
      }
  ...
  public void setDefaultFetchSize(int fetchSize) throws SQLException {
    if (fetchSize < 0) {
      throw new KSQLException(GT.tr("Fetch size must be a value greater to or equal to 0."),
        KSQLState.INVALID_PARAMETER_VALUE);
    }
  ...
  @Override
  public KWReplicationConnection getReplicationAPI() {
    return new KWReplicationConnectionImpl(this);
  }
  ...
  private static void appendArray(StringBuilder sb, Object elements, char delim) {
    ...
    for (int i = 0; i < nElements; i++) {
      ...
      if (o == null) {
        ...
        String s = o.toString();
        KwArray.escapeArrayElement(sb, s);
      }
    }
    sb.append('}');
  }
  ...
  public Statement createStatement(int resultSetType, int resultSetConcurrency,
    ...
    checkClosed();
    return new KwStatement(this, resultSetType, resultSetConcurrency, resultSetHoldability);
  }
  ...
  public PreparedStatement prepareStatement(String sql, int resultSetType, int resultSetConcurrency,
                                            int resultSetHoldability) throws SQLException {
    checkClosed();
    return new KwPreparedStatement(this, sql, resultSetType, resultSetConcurrency,
  ...
  public CallableStatement prepareCall(String sql, int resultSetType, int resultSetConcurrency,
                                       int resultSetHoldability) throws SQLException {
    checkClosed();
    return new KwCallableStatement(this, sql, resultSetType, resultSetConcurrency,
      resultSetHoldability);
  }
  ...
  public DatabaseMetaData getMetaData() throws SQLException {
    checkClosed();
    if (metadata == null) {
      metadata = new KwDatabaseMetaData(this);
    }
    return metadata;
  }
...
//    return makeSQLXML();
  ...
  public Array createArrayOf(String typeName, Object elements) throws SQLException {
    ...
    final int oid = typeInfo.getKWArrayType(typeName);
    final char delim = typeInfo.getArrayDelimiter(oid);

    if (oid == Oid.UNSPECIFIED) {
      throw new KSQLException(GT.tr("Unable to find server array type for provided name {0}.", typeName),
        KSQLState.INVALID_NAME);
    }
    ...
    if (arraySupport != null) {
      ...
      if (oid == arraySupport.getDefaultArrayTypeOid(typeInfo) && arraySupport.supportBinaryRepresentation()
        ...
        return new KwArray(this, oid, arraySupport.toBinaryRepresentation(this, elements));
      }
  ...
  public Array createArrayOf(String typeName, Object[] elements) throws SQLException {
    ...
    int oid = getTypeInfo().getKWArrayType(typeName);
  ...
  public boolean isValid(int timeout) throws SQLException {
    if (timeout < 0) {
      throw new KSQLException(GT.tr("Invalid timeout ({0}<0).", timeout),
        KSQLState.INVALID_PARAMETER_VALUE);
    }
    if (isClosed()) {
      return false;
  ...
  public void setClientInfo(String name, String value) throws SQLClientInfoException {
    ...
    addWarning(new SQLWarning(GT.tr("ClientInfo property not supported."),
      KSQLState.NOT_IMPLEMENTED.getState()));
  }
  ...
  public void setClientInfo(Properties properties) throws SQLClientInfoException {
    ...
    if (!failures.isEmpty()) {
      throw new SQLClientInfoException(GT.tr("One or more ClientInfo failed."),
        KSQLState.NOT_IMPLEMENTED.getState(), failures);
    }
  }
  ...
  public void setNetworkTimeout(Executor executor /*not used*/, int milliseconds) throws SQLException {
    ...
    if (milliseconds < 0) {
      throw new KSQLException(GT.tr("Network timeout must be a value greater than or equal to 0."),
        KSQLState.INVALID_PARAMETER_VALUE);
    }
    ...
    try {
      ...
      throw new KSQLException(GT.tr("Unable to set network timeout."),
        KSQLState.COMMUNICATION_ERROR, ioe);
    }
  }
  ...
  public int getNetworkTimeout() throws SQLException {
    ...
    try {
      ...
      throw new KSQLException(GT.tr("Unable to get network timeout."),
        KSQLState.COMMUNICATION_ERROR, ioe);
    }
  }
...
//        break;
//      default:
//        throw new KSQLException(GT.tr("Unknown ResultSet holdability setting: {0}.", holdability),
//            PSQLState.INVALID_PARAMETER_VALUE);
//    }
//    LOGGER.log(Level.FINE, "  setHoldability = {0}", holdability);
    throw Driver.notImplemented(this.getClass(), "setHoldability(int)");
  }
  ...
  public Savepoint setSavepoint() throws SQLException {
    ...
    String kwName;
    if (getAutoCommit()) {
      throw new KSQLException(GT.tr("Cannot establish a savepoint in auto-commit mode."),
        KSQLState.NO_ACTIVE_SQL_TRANSACTION);
    }

    KSQLSavepoint savepoint = new KSQLSavepoint(savepointId++);
    kwName = savepoint.getKWName();

    // Note we can't use execSQLUpdate because we don't want
    // to suppress BEGIN.
    Statement stmt = createStatement();
    stmt.executeUpdate("SAVEPOINT " + kwName);
    stmt.close();
  ...
  public Savepoint setSavepoint(String name) throws SQLException {
    ...
    KSQLSavepoint savepoint = new KSQLSavepoint(name);

    // Note we can't use execSQLUpdate because we don't want
    // to suppress BEGIN.
    Statement stmt = createStatement();
    stmt.executeUpdate("SAVEPOINT " + savepoint.getKWName());
    stmt.close();
  ...
  public void rollback(Savepoint savepoint) throws SQLException {
    ...
    KSQLSavepoint kwSavepoint = (KSQLSavepoint) savepoint;
    execSQLUpdate("ROLLBACK TO SAVEPOINT " + kwSavepoint.getKWName());
  }
  ...
  public void releaseSavepoint(Savepoint savepoint) throws SQLException {
    ...
    KSQLSavepoint kwSavepoint = (KSQLSavepoint) savepoint;
    execSQLUpdate("RELEASE SAVEPOINT " + kwSavepoint.getKWName());
    kwSavepoint.invalidate();
  }
  ...
  public PreparedStatement prepareStatement(String sql, int[] columnIndexes) throws SQLException {
    ...
    checkClosed();
    throw new KSQLException(GT.tr("Returning autogenerated keys is not supported."),
      KSQLState.NOT_IMPLEMENTED);
  }
  ...
  public PreparedStatement prepareStatement(String sql, String[] columnNames) throws SQLException {
    ...
    KwPreparedStatement ps =
      new KwPreparedStatement(this, cachedQuery,
