...
public abstract class AbstractBlobClob {
  ...
  public synchronized void truncate(long len) throws SQLException {
    ...
    if (len < 0) {
      throw new KSQLException(GT.tr("Cannot truncate LOB to a negative length."),
    ...
    if (len > Integer.MAX_VALUE) {
      if (support64bit) {
        ...
        throw new KSQLException(GT.tr("KaiwuDB LOBs can only index to: {0}", Integer.MAX_VALUE),
            KSQLState.INVALID_PARAMETER_VALUE);
      }
    } else {
  ...
  protected void assertPosition(long pos, long len) throws SQLException {
    checkFreed();
    if (pos < 1) {
      throw new KSQLException(GT.tr("LOB positioning offsets start at 1."),
          KSQLState.INVALID_PARAMETER_VALUE);
    }
  ...
  protected void checkFreed() throws SQLException {
    if (subLOs == null) {
      throw new KSQLException(GT.tr("free() was called on this LOB previously"),
