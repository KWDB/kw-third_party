...
/**
 * <p>Use Waffle-JNI to support SSPI authentication when KwJDBC is running on a Windows client and
...
public class SSPIClient implements ISSPIClient {
  ...
  public static final String SSPI_DEFAULT_SPN_SERVICE_CLASS = "KAIWUDB";
   ...
   *
   * @param kwStream KaiwuDB connection stream
   * @param spnServiceClass SSPI SPN service class, defaults to KAIWUDB if null
   * @param enableNegotiate enable negotiate
   */
  public SSPIClient(KWStream kwStream, String spnServiceClass, boolean enableNegotiate) {
  ...
  private String makeSPN() throws KSQLException {
    final HostSpec hs = kwStream.getHostSpec();

    try {
      return NTDSAPIWrapper.instance.DsMakeSpn(spnServiceClass, hs.getHost(), null,
          (short) hs.getPort(), null);
    } catch (LastErrorException ex) {
      throw new KSQLException("SSPI setup failed to determine SPN",
          KSQLState.CONNECTION_UNABLE_TO_CONNECT, ex);
    }
  }
  ...
  public void startSSPI() throws SQLException, IOException {
    ...
    try {
       ...
       *
       * This corresponds to kw_SSPI_startup in libpq/fe-auth.c .
       */
      try {
        clientCredentials = WindowsCredentialsHandleImpl.getCurrent(securityPackage);
        clientCredentials.initialize();
      } catch (Win32Exception ex) {
        throw new KSQLException("Could not obtain local Windows credentials for SSPI",
            KSQLState.CONNECTION_UNABLE_TO_CONNECT /* TODO: Should be authentication error */, ex);
      }
      ...
      try {
        ...
        throw new KSQLException("Could not initialize SSPI security context",
            KSQLState.CONNECTION_UNABLE_TO_CONNECT /* TODO: Should be auth error */, ex);
      }
  ...
  public void continueSSPI(int msgLength) throws SQLException, IOException {
    ...
    byte[] receivedToken = kwStream.receive(msgLength);
