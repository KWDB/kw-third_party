...
public class messages_bg extends java.util.ResourceBundle {
  ...
  static {
    ...
    t[26] = "The authentication type {0} is not supported. Check that you have configured the kw_hba.conf file to include the client''s IP address or subnet, and that it is using an authentication scheme supported by the driver.";
    t[27] = "Тип на удостоверяване {0} не се поддържа. Проверете дали сте конфигурирали kw_hba.conf файла, да включва IP адреса на клиента или подмрежата, и че се използва схема за удостоверяване, поддържана от драйвъра.";
    ...
    t[292] = "KaiwuDB LOBs can only index to: {0}";
    t[293] = "KaiwuDB индексира големи обекти LOB само до: {0}";
