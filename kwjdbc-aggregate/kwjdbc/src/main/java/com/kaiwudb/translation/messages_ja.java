...
public class messages_ja extends java.util.ResourceBundle {
  ...
  static {
    ...
    t[412] = "The authentication type {0} is not supported. Check that you have configured the kw_hba.conf file to include the client''s IP address or subnet, and that it is using an authentication scheme supported by the driver.";
    t[413] = "認証タイプ {0} はサポートされません。kw_hba.confでクライアントのIPアドレスまたはサブネットの指定があり、そのエントリでこのドライバがサポートする認証機構を使うように設定されていることを確認してください。";
    ...
    t[1234] = "SCRAM authentication is not supported by this driver. You need JDK >= 8 and kwjdbc >= 42.2.0 (not \".jre\" versions)";
    t[1235] = "SCRAM認証はこのドライバではサポートされません。JDK8 以降かつ kwjdbc 42.2.0 以降(\".jre\"のバージョンではありません)が必要です。";
    ...
    t[1308] = "KaiwuDB LOBs can only index to: {0}";
    t[1309] = "KaiwuDB LOB 上の位置指定は最大 {0} までです";
    t[1310] = "Where: {0}";
    t[1311] = "場所: {0}";
    ...
    t[1376] = " (kwjdbc: autodetected server-encoding to be {0}, if the message is not readable, please check database logs and/or host, port, dbname, user, password, kw_hba.conf)";
    t[1377] = "(kwjdbc: server-encoding として {0}  を自動検出しました、メッセージが読めない場合はデータベースログおよび host, port, dbname, user, password, kw_dba.conf を確認してください)";
