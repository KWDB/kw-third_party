...
/**
 * Converts to and from the kaiwudb bytea datatype used by the backend.
 */
public class KWbytea {
  private static final int MAX_3_BUFF_SIZE = 2 * 1024 * 1024;

  /*
   * Converts a KW bytea raw value (i.e. the raw binary representation of the bytea data type) into
   * a java byte[]
   */
  public static byte[] toBytes(byte[] s) throws SQLException {
    ...
    // Starting with KW 9.0, a new hex format is supported
  ...
  /*
   * Converts a java byte[] into a KW bytea string (i.e. the text representation of the bytea data
   * type)
   */
  public static String toKWString(byte[] buf) {
    if (buf == null) {
      return null;
    }
