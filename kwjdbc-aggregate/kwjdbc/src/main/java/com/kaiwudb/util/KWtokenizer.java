...
/**
 * This class is used to tokenize the text output of com.kaiwudb. It's mainly used by the geometric
 * classes, but is useful in parsing any output from custom data types output from com.kaiwudb.
 *
 * @see KWbox
 * @see KWcircle
 * @see KWlseg
...
public class KWtokenizer {
   ...
   */
  public KWtokenizer(String string, char delim) {
  ...
  public int tokenize(String string, char delim) {
    ...
    // (usualls KWpoint) imbedded within a token.
    //
   ...
   *
   * <p>The geometric datatypes use this to process nested tokens (usually KWpoint).</p>
   *
   * @param n Token number ( 0 ... getSize()-1 )
   * @param delim The delimiter to use
   * @return A new instance of KWtokenizer based on the token
   */
  public KWtokenizer tokenizeToken(int n, char delim) {
    return new KWtokenizer(getToken(n), delim);
  }
