...
import com.kaiwudb.util.StreamWrapper;
...
import com.kaiwudb.jdbc.UUIDArrayAssistant;
...
class SimpleParameterList implements V3ParameterList {
  ...
  public String toString(int index, boolean standardConformingStrings) {
    ...
    if (paramValues[index] == null) {
      ...
      switch (paramTypes[index]) {
        ...
        case Oid.POINT:
          KWpoint kwPoint = new KWpoint();
          kwPoint.setByteValue((byte[]) paramValues[index], 0);
          return "'" + kwPoint.toString() + "'::point";

        case Oid.BOX:
          KWbox kwBox = new KWbox();
          kwBox.setByteValue((byte[]) paramValues[index], 0);
          return "'" + kwBox.toString() + "'::box";
      }
      return "?";
    } else {
  ...
  public void checkAllParametersSet() throws SQLException {
    for (int i = 0; i < paramTypes.length; ++i) {
      if (direction(i) != OUT && paramValues[i] == null) {
        throw new KSQLException(GT.tr("No value specified for parameter {0}.", i + 1),
            KSQLState.INVALID_PARAMETER_VALUE);
      }
    }
  }
  ...
  private static void streamBytea(KWStream kwStream, StreamWrapper wrapper) throws IOException {
    byte[] rawData = wrapper.getBytes();
    if (rawData != null) {
      kwStream.send(rawData, wrapper.getOffset(), wrapper.getLength());
      return;
    }

    kwStream.sendStream(wrapper.getStream(), wrapper.getLength());
  }
  ...
  void writeV3Value(int index, KWStream kwStream) throws IOException {
    --index;
    ...
    // Directly encoded?
    if (paramValues[index] instanceof byte[]) {
      kwStream.send((byte[]) paramValues[index]);
      return;
    }

    // Binary-format bytea?
    if (paramValues[index] instanceof StreamWrapper) {
      streamBytea(kwStream, (StreamWrapper) paramValues[index]);
      return;
    }
