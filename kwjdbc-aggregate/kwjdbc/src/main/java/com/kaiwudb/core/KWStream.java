 ...
 *
 * <p>In general, instances of KWStream are not threadsafe; the caller must ensure that only one thread
 * at a time is accessing a particular KWStream instance.</p>
 */
public class KWStream implements Closeable, Flushable {
  ...
  /**
   * Constructor: Connect to the KaiwuDB back end and return a stream connection.
   *
   ...
   * @param hostSpec the host and port to connect to
   * @throws IOException if an IOException occurs below it.
   * @deprecated use {@link #KWStream(SocketFactory, HostSpec, int)}
   */
  @Deprecated
  ...
  public void changeSocket(Socket socket) throws IOException {
    ...
    kwInput = new VisibleBufferedInputStream(connection.getInputStream(), 8192);
    kwOutput = new BufferedOutputStream(connection.getOutputStream(), 8192);
  ...
  public void setEncoding(Encoding encoding) throws IOException {
    ...
    // will call KWStream.flush() as needed.
    OutputStream interceptor = new FilterOutputStream(kwOutput) {
      @Override
   ...
   * called when switching between use of the Writer and use of the KWStream write methods, but it
  ...
  public void send(byte[] buf, int off, int siz) throws IOException {
    ...
    kwOutput.write(buf, off, bufamt < siz ? bufamt : siz);
  ...
  public String receiveString(int len) throws IOException {
    if (!kwInput.ensureBytes(len)) {
      throw new EOFException();
    }

    String res = encoding.decode(kwInput.getBuffer(), kwInput.getIndex(), len);
    kwInput.skip(len);
    return res;
  }
  ...
  public EncodingPredictor.DecodeResult receiveErrorString(int len) throws IOException {
    ...
    try {
      String value = encoding.decode(kwInput.getBuffer(), kwInput.getIndex(), len);
      // no autodetect warning as the message was converted on its own
      res = new EncodingPredictor.DecodeResult(value, null);
    } catch (IOException e) {
      res = EncodingPredictor.decode(kwInput.getBuffer(), kwInput.getIndex(), len);
      if (res == null) {
        Encoding enc = Encoding.defaultEncoding();
        String value = enc.decode(kwInput.getBuffer(), kwInput.getIndex(), len);
  ...
  public String receiveString() throws IOException {
    int len = kwInput.scanCStringLength();
    String res = encoding.decode(kwInput.getBuffer(), kwInput.getIndex(), len - 1);
    kwInput.skip(len);
    return res;
  }
  ...
  public void receive(byte[] buf, int off, int siz) throws IOException {
    ...
    while (s < siz) {
      int w = kwInput.read(buf, off + s, siz - s);
      if (w < 0) {
        throw new EOFException();
      }
      s += w;
  ...
  public void receiveEOF() throws SQLException, IOException {
    ...
    throw new KSQLException(GT.tr("Expected an EOF from server, got: {0}", c),
