...
{
	return appendStringInfoCharMacro(buf, byt);
}
...
{
	return appendBinaryStringInfo(buf, data, datalen);
}
				   ...
				   bool countincludesself)
{
	...
	if (pq_sendint(buf, slen + extra, 4) != STATUS_OK) {
		return STATUS_ERROR;
	}	
	return appendBinaryStringInfo(buf, str, slen);
}
...
pq_sendstring(StringInfo buf, const char *str)
{
	...
	return appendBinaryStringInfo(buf, str, slen + 1);
}
...
pq_send_ascii_string(StringInfo buf, const char *str)
{
	while (*str)
	{
		...
		if (IS_HIGHBIT_SET(ch))
			ch = '?';
		if (appendStringInfoCharMacro(buf, ch) != STATUS_OK) {
			return STATUS_ERROR;
	}
	}
	return appendStringInfoChar(buf, '\0');
}
...
pq_sendint(StringInfo buf, int i, int b)
{
	...
	switch (b)
	{
		case 1:
			n8 = (unsigned char) i;
			return appendBinaryStringInfo(buf, (char *) &n8, 1);
		case 2:
			n16 = htons((uint16) i);
			return appendBinaryStringInfo(buf, (char *) &n16, 2);
		case 4:
			n32 = htonl((uint32) i);
			return appendBinaryStringInfo(buf, (char *) &n32, 4);
		default:
		  	// TODO(SH): log
...
pq_sendint64(StringInfo buf, int64 i)
{
	...
	n32 = (uint32) (i >> 32);
	n32 = htonl(n32);
	if (appendBinaryStringInfo(buf, (char *) &n32, 4) != STATUS_OK) {
		return STATUS_ERROR;
	}
...
pq_sendfloat4(StringInfo buf, float4 f)
{
	...
	return appendBinaryStringInfo(buf, (char *) &swap.i, 4);
}
...
pq_sendfloat8(StringInfo buf, float8 f)
{
	...
	swap.f = f;
	return pq_sendint64(buf, swap.i);
}
 ...
 */
void
pq_endmessage(Port *port, StringInfo buf)
{
	/* msgtype was saved in cursor field */
	(void) pq_putmessage(port, buf->cursor, buf->data, buf->len);
...
{
	if (initStringInfo(buf) != 0) {
		return STATUS_ERROR;
	}
	/* Reserve four bytes for the bytea length word */
	if (appendStringInfoCharMacro(buf, '\0') != 0) return STATUS_ERROR;
	if (appendStringInfoCharMacro(buf, '\0') != 0) return STATUS_ERROR;
	if (appendStringInfoCharMacro(buf, '\0') != 0) return STATUS_ERROR;
	if (appendStringInfoCharMacro(buf, '\0') != 0) return STATUS_ERROR;
	return STATUS_OK;
 ...
 */
void
pq_puttextmessage(Port *port, char msgtype, const char *str)
{
	int			slen = strlen(str);

	(void) pq_putmessage(port, msgtype, str, slen + 1);
}
 ...
 */
void
pq_putemptymessage(Port *port, char msgtype)
{
	(void) pq_putmessage(port, msgtype, NULL, 0);
}
