...
static void my_BIO_s_socket(BIO_METHOD **my_bio_methods);
...
89AHxstDqKSt90xkhkn4DIO9ZekX1KHTUPj1WV/cdlJPPT2N286Z4VeSWc39uK50\n\
T8X8dryDxUcwYc58yWb/Ffm7/KFexwGq01uejaClcjrUGvC/RgBYK+X0iP1YTknb\n\
...
ERRMp5QqOaHJkM+Dxv8Cj6MqrCbfC4u+KErxodzuusgDgvZiLF22uxMZbobFWyte\n\
...
//                      SSLerrmessage(ERR_get_error()))));
  ...
  SSL_CTX_set_options(SSL_context,
                      ...
                      SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1 | SSL_OP_NO_TLSv1);
...
//                           err)));
  ...
  if (port->peer != NULL)
  {
    ...
    if (len != -1)
    {
      ...
      peer_cn = static_cast<char *>(malloc(len + 1));
      if (peer_cn == NULL) 
        return -1;
      ...
      /*
       * Reject embedded NULLs in certificate include name to prevent
...
//        ereport(COMMERROR,
//                (errcode(ERRCODE_PROTOCOL_VIOLATION),
//                    errmsg("SSL certificate's include name contains embedded null")));
        free(peer_cn);
        return -1;
      }
...
be_tls_close(Port *port)
{
  ...
  if (port->bio_method)
  {
    BIO_meth_free(port->bio_method);
    port->bio_method = NULL;
  }
}
 ...
 */
bool
be_tls_read_pending(Port *port)
{
	return SSL_pending(port->ssl) > 0;
}
...
static void
my_BIO_s_socket(BIO_METHOD **my_bio_methods)
{
  if (!*my_bio_methods)
  {
...
my_SSL_set_fd(Port *port, int fd)
{
  ...
  my_BIO_s_socket(&port->bio_method);
  if (port->bio_method == NULL)
  {
    SSLerr(SSL_F_SSL_SET_FD, ERR_R_BUF_LIB);
    goto err;
  }
  bio = BIO_new(port->bio_method);
