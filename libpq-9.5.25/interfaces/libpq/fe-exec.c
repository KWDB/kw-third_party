...
/*
 * AEexec -- like PQputnbytes && flush && exec, buffer send to server exec
 * and wait for result.
 * Returns PGresult if exec ok, nil if not.
 */
PGresult *AEexec(PGconn *conn, const char *buffer, int nbytes)
{
  if (!conn)
  {
    return NULL;
  ...
  if (nbytes > 0)
  {
    if ((conn->outBufSize - conn->outCount - 5) < nbytes)
    {
      ...
      {
        return pqIsnonblocking(conn) ? 0 : NULL;
      }
    }
    if (!PQexecStart(conn))
	  {
    ...
    if (pqPutMsgStart('Q', false, conn) < 0 ||
        pqPutInt(nbytes, sizeof(int), conn) < 0 ||
        pqPutnchar(buffer, nbytes, conn) < 0 || pqPutMsgEnd(conn) < 0)
    {
      return NULL;
    }
    if (pqFlush(conn) < 0)
