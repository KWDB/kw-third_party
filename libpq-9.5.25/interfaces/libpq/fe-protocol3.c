...
static void reportErrorPosition(PQExpBuffer msg, const char *query, int loc,
                                int encoding);
static int build_startup_packet(const PGconn *conn, char *packet,
                                const PQEnvironmentOption *options);
static int pqSaveAEInfo(PGconn *conn, int msgLength);
...
int pqGetErrorNotice3(PGconn *conn, bool isError) {
   ...
   */
  if (isError) pqClearAsyncResult(conn);
   ...
   */
  for (;;) {
    if (pqGetc(&id, conn)) goto fail;
    if (id == '\0') break; /* terminator found */
    if (pqGets(&workBuf, conn)) goto fail;
  ...
  if (val) appendPQExpBuffer(&workBuf, "%s:  ", val);
  ...
  if (val) appendPQExpBufferStr(&workBuf, val);
  ...
  if (conn->verbosity != PQERRORS_TERSE) {
    ...
    if (val) appendPQExpBuffer(&workBuf, libpq_gettext("DETAIL:  %s\n"), val);
    val = PQresultErrorField(res, PG_DIAG_MESSAGE_HINT);
    if (val) appendPQExpBuffer(&workBuf, libpq_gettext("HINT:  %s\n"), val);
    val = PQresultErrorField(res, PG_DIAG_INTERNAL_QUERY);
    if (val) appendPQExpBuffer(&workBuf, libpq_gettext("QUERY:  %s\n"), val);
    val = PQresultErrorField(res, PG_DIAG_CONTEXT);
    if (val) appendPQExpBuffer(&workBuf, libpq_gettext("CONTEXT:  %s\n"), val);
  }
  if (conn->verbosity == PQERRORS_VERBOSE) {
    ...
    if (val)
      appendPQExpBuffer(&workBuf, libpq_gettext("DATATYPE NAME:  %s\n"), val);
  ...
  if (conn->verbosity == PQERRORS_VERBOSE) {
    ...
    if (val || valf || vall) {
      ...
      if (val) appendPQExpBuffer(&workBuf, libpq_gettext("%s, "), val);
   ...
   */
  if (isError) {
    if (res) res->errMsg = pqResultStrdup(res, workBuf.data);
...
#define MIN_RIGHT_CUT 10 /* try to keep this far away from EOL */
  ...
  wquery = strdup(query);
  if (wquery == NULL) return; /* fail silently if out of memory */
  ...
  for (cno = 0; wquery[qoffset] != '\0'; cno++) {
     ...
     */
      ...
      if (cno < loc) {
        if (ch == '\r' || cno == 0 || wquery[qidx[cno - 1]] != '\r') loc_line++;
  ...
  if (loc <= cno) {
    ...
    if (scridx[iend] - scridx[ibeg] > DISPLAY_SIZE) {
      ...
      if (scridx[ibeg] + DISPLAY_SIZE >= scridx[loc] + MIN_RIGHT_CUT) {
        while (scridx[iend] - scridx[ibeg] > DISPLAY_SIZE) iend--;
        end_trunc = true;
      } else {
    ...
    if (beg_trunc) appendPQExpBufferStr(msg, "...");
    ...
    if (end_trunc) appendPQExpBufferStr(msg, "...");
    appendPQExpBufferChar(msg, '\n');

    /* Now emit the cursor marker line. */
    scroffset += scridx[loc] - scridx[ibeg];
    for (i = 0; i < scroffset; i++) appendPQExpBufferChar(msg, ' ');
    appendPQExpBufferChar(msg, '^');
    appendPQExpBufferChar(msg, '\n');
  }
...
static int pqSaveAEInfo(PGconn *conn, int msgLength) {
  int count;
  if (pqGetInt(&count, 4, conn)) {
    return 1;
  }
  aeParameters *p = (aeParameters *)malloc(sizeof(aeParameters)), *prev = NULL;
  conn->aeBuffer = p;
  for (int i = 0; i < count; i++) {
    int key_length = 0, val_length = 0;
    pqGetInt(&key_length, 4, conn);
    p->key = (char *)malloc(key_length + 1);
    pqGetnchar(p->key, key_length + 1, conn);

    p->oid = 0;
    pqGetInt(&p->oid, 4, conn);

    pqGetInt(&val_length, 4, conn);
    p->value = (char *)malloc(val_length + 1);
    pqGetnchar(p->value, val_length + 1, conn);

    p->next = NULL;
    prev = p;
    p = p->next;
    if (p == NULL && i != count - 1) {
      p = (aeParameters *)malloc(sizeof(aeParameters));
      p->next = NULL;
    }
    prev->next = p;
...
int pqGetCopyData3(PGconn *conn, char **buffer, int async) {
  ...
  for (;;) {
    ...
    if (msgLength < 0) return msgLength; /* end-of-copy or error */
    if (msgLength == 0) {
...
int pqGetlineAsync3(PGconn *conn, char *buffer, int bufsize) {
   ...
   */
  msgLength = getCopyDataMessage(conn);
  if (msgLength < 0) return -1; /* end-of-copy or error */
  if (msgLength == 0) return 0; /* no data yet */
...
int pqEndcopy3(PGconn *conn) {
  ...
  if (conn->asyncStatus == PGASYNC_COPY_IN ||
      conn->asyncStatus == PGASYNC_COPY_BOTH) {
    if (pqPutMsgStart('c', false, conn) < 0 || pqPutMsgEnd(conn) < 0) return 1;
   ...
   * and the flush fails
   */
  if (pqFlush(conn) && pqIsnonblocking(conn)) return 1;
   ...
   */
  if (pqIsnonblocking(conn) && PQisBusy(conn)) return 1;
 ...
 * See fe-exec.c for documentation.
 */
PGresult *pqFunctionCall3(PGconn *conn, Oid fnid, int *result_buf,
  ...
  for (i = 0; i < nargs; ++i) { /* len.int4 + contents	   */
    ...
    if (args[i].len == -1) continue; /* it's NULL */
  ...
  for (;;) {
    if (needInput) {
      ...
      if (pqWait(TRUE, FALSE, conn) || pqReadData(conn) < 0) break;
    }
    ...
    conn->inCursor = conn->inStart;
    if (pqGetc(&id, conn)) continue;
    if (pqGetInt(&msgLength, 4, conn)) continue;
    ...
    switch (id) {
      case 'V': /* function result */
        if (pqGetInt(actual_result_len, 4, conn)) continue;
        ...
        break;
      case 'E': /* error return */
        if (pqGetErrorNotice3(conn, true)) continue;
        status = PGRES_FATAL_ERROR;
        break;
      case 'A': /* notify message */
        /* handle notify and go back to processing return values */
        if (getNotify(conn)) continue;
        break;
      case 'N': /* notice */
        /* handle notice and go back to processing return values */
        if (pqGetErrorNotice3(conn, false)) continue;
        break;
      case 'Z': /* backend is ready for new query */
        if (getReadyForQuery(conn)) continue;
        /* consume the message and exit */
        conn->inStart += 5 + msgLength;
        /* if we saved a result object (probably an error), use it */
        if (conn->result) return pqPrepareAsyncResult(conn);
        return PQmakeEmptyPGresult(conn, status);
      case 'S': /* parameter status */
        if (getParameterStatus(conn)) continue;
        break;
      default:
...
#define ADD_STARTUP_OPTION(optname, optval)           \
  do {                                                \
    if (packet) strcpy(packet + packet_len, optname); \
    packet_len += strlen(optname) + 1;                \
    if (packet) strcpy(packet + packet_len, optval);  \
    packet_len += strlen(optval) + 1;                 \
  } while (0)

  if (conn->pguser && conn->pguser[0]) ADD_STARTUP_OPTION("user", conn->pguser);
  if (conn->dbName && conn->dbName[0])
    ADD_STARTUP_OPTION("database", conn->dbName);

	if (conn->conntype && conn->conntype[0])
		ADD_STARTUP_OPTION("conntype", conn->conntype);
	if (conn->token && conn->token[0])
		ADD_STARTUP_OPTION("token", conn->token);
  if (conn->instance && conn->instance[0])
    ADD_STARTUP_OPTION("instance", conn->instance);
  if (conn->mode && conn->mode[0]) ADD_STARTUP_OPTION("mode", conn->mode);
  if (conn->conn_id && conn->conn_id[0])
    ADD_STARTUP_OPTION("conn_id", conn->conn_id);
  ...
  if (conn->send_appname) {
    ...
    if (val && val[0]) ADD_STARTUP_OPTION("application_name", val);
  }
