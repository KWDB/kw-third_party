...
static const internalPQconninfoOption PQconninfoOptions[] = {
     ...
     * still try to set it.
     */
    {"authtype", "PGAUTHTYPE", DefaultAuthtype, NULL, "Database-Authtype", "D",
     20, -1},
  ...
  "inner-Conn-password", "", 64, offsetof(struct pg_conn, token)},

  {"conntype", NULL, NULL, NULL,
  "Conn-type", "", 20, offsetof(struct pg_conn, conntype)},
    {"user", "PGUSER", NULL, NULL, "Database-User", "", 20,
     offsetof(struct pg_conn, pguser)},

    {"password", "PGPASSWORD", NULL, NULL, "Database-Password", "*", 20,
     offsetof(struct pg_conn, pgpass)},

    {"connect_timeout", "PGCONNECT_TIMEOUT", NULL, NULL, "Connect-timeout", "",
     10, /* strlen(INT32_MAX) == 10 */
     offsetof(struct pg_conn, connect_timeout)},

    {"dbname", "PGDATABASE", NULL, NULL, "Database-Name", "", 20,
     offsetof(struct pg_conn, dbName)},

    {"host", "PGHOST", NULL, NULL, "Database-Host", "", 40,
     offsetof(struct pg_conn, pghost)},

    {"hostaddr", "PGHOSTADDR", NULL, NULL, "Database-Host-IP-Address", "", 45,
     offsetof(struct pg_conn, pghostaddr)},

    {"port", "PGPORT", DEF_PGPORT_STR, NULL, "Database-Port", "", 6,
     offsetof(struct pg_conn, pgport)},

    {"client_encoding", "PGCLIENTENCODING", NULL, NULL, "Client-Encoding", "",
     10, offsetof(struct pg_conn, client_encoding_initial)},

    /*
     * "tty" is no longer used either, but keep it present for backwards
     * compatibility.
     */
    {"tty", "PGTTY", DefaultTty, NULL, "Backend-Debug-TTY", "D", 40,
     offsetof(struct pg_conn, pgtty)},

    {"options", "PGOPTIONS", DefaultOption, NULL, "Backend-Debug-Options", "D",
     40, offsetof(struct pg_conn, pgoptions)},

    {"application_name", "PGAPPNAME", NULL, NULL, "Application-Name", "", 64,
     offsetof(struct pg_conn, appname)},

    {"fallback_application_name", NULL, NULL, NULL, "Fallback-Application-Name",
     "", 64, offsetof(struct pg_conn, fbappname)},

    {"keepalives", NULL, NULL, NULL, "TCP-Keepalives", "",
     1, /* should be just '0' or '1' */
     offsetof(struct pg_conn, keepalives)},

    {"keepalives_idle", NULL, NULL, NULL, "TCP-Keepalives-Idle", "",
     10, /* strlen(INT32_MAX) == 10 */
     offsetof(struct pg_conn, keepalives_idle)},

    {"keepalives_interval", NULL, NULL, NULL, "TCP-Keepalives-Interval", "",
     10, /* strlen(INT32_MAX) == 10 */
     offsetof(struct pg_conn, keepalives_interval)},

    {"keepalives_count", NULL, NULL, NULL, "TCP-Keepalives-Count", "",
     10, /* strlen(INT32_MAX) == 10 */
     ...
     */
    {"sslmode", "PGSSLMODE", DefaultSSLMode, NULL, "SSL-Mode", "",
     12, /* sizeof("verify-full") == 12 */
     offsetof(struct pg_conn, sslmode)},

    {"sslcompression", "PGSSLCOMPRESSION", "1", NULL, "SSL-Compression", "", 1,
     offsetof(struct pg_conn, sslcompression)},

    {"sslcert", "PGSSLCERT", NULL, NULL, "SSL-Client-Cert", "", 64,
     offsetof(struct pg_conn, sslcert)},

    {"sslkey", "PGSSLKEY", NULL, NULL, "SSL-Client-Key", "", 64,
     offsetof(struct pg_conn, sslkey)},

    {"sslrootcert", "PGSSLROOTCERT", NULL, NULL, "SSL-Root-Certificate", "", 64,
     offsetof(struct pg_conn, sslrootcert)},

    {"sslcrl", "PGSSLCRL", NULL, NULL, "SSL-Revocation-List", "", 64,
     offsetof(struct pg_conn, sslcrl)},

    {"requirepeer", "PGREQUIREPEER", NULL, NULL, "Require-Peer", "", 10,
    ...
    {"krbsrvname", "PGKRBSRVNAME", PG_KRB_SRVNAM, NULL, "Kerberos-service-name",
     "", 20, offsetof(struct pg_conn, krbsrvname)},

    {"gsslib", "PGGSSLIB", NULL, NULL, "GSS-library", "",
     7, /* sizeof("gssapi") = 7 */
     offsetof(struct pg_conn, gsslib)},

    {"replication", NULL, NULL, NULL, "Replication", "D", 5,
     offsetof(struct pg_conn, replication)},

    // KWDB extend connection options

    {"instance", "KWDBINSTANCE", NULL, NULL, "Instance-Name", "", 20,
     offsetof(struct pg_conn, instance)},

    {"mode", "KWDBMODE", NULL, NULL, "Mode", "", 10,
     offsetof(struct pg_conn, mode)},

    {"conn_id", "KDCONNECTIONID", NULL, NULL, "Connection-ID", "", 10,
     offsetof(struct pg_conn, conn_id)},
...
static PQconninfoOption *conninfo_storeval(PQconninfoOption *connOptions,
                                           ...
                                           const char *value,
                                           PQExpBuffer errorMessage,
                                           bool ignoreMissing, bool uri_decode);
...
void pqDropConnection(PGconn *conn, bool flushInput) {
  ...
  if (conn->sock != PGINVALID_SOCKET) closesocket(conn->sock);
  conn->sock = PGINVALID_SOCKET;

  /* Optionally discard any unread data */
  if (flushInput) conn->inStart = conn->inCursor = conn->inEnd = 0;
...
#ifdef ENABLE_GSS
  {
    ...
    if (conn->gtarg_nam) gss_release_name(&min_s, &conn->gtarg_nam);
    if (conn->ginbuf.length) gss_release_buffer(&min_s, &conn->ginbuf);
    if (conn->goutbuf.length) gss_release_buffer(&min_s, &conn->goutbuf);
  }
#endif
#ifdef ENABLE_SSPI
  if (conn->ginbuf.length) free(conn->ginbuf.value);
  conn->ginbuf.length = 0;
  conn->ginbuf.value = NULL;
  if (conn->sspitarget) free(conn->sspitarget);
 ...
 */
PGPing PQpingParams(const char *const *keywords, const char *const *values,
                    int expand_dbname) {
  PGconn *conn = PQconnectStartParams(keywords, values, expand_dbname);
  PGPing ret;
...
PGconn *PQconnectStartParams(const char *const *keywords,
   ...
   */
  connOptions = conninfo_array_parse(keywords, values, &conn->errorMessage,
                                     true, expand_dbname);
  if (connOptions == NULL) {
...
static bool connectOptions2(PGconn *conn) {
  ...
  if (conn->dbName == NULL || conn->dbName[0] == '\0') {
	...
	if (conn->pgpass == NULL || conn->pgpass[0] == '\0')
	{
		if (conn->pgpass)
			free(conn->pgpass);
		// if inner connection, should use token as password
    if(conn->conntype != NULL && CONNECT_INNER == atoi(conn->conntype)) {
        if ((conn->token != NULL) && (conn->token[0] != '\0')) {
            conn->pgpass = strdup(conn->token);
        }
    } else {
      conn->pgpass = PasswordFromFile(conn->pghost, conn->pgport, conn->dbName,
                                      conn->pguser);
      if (conn->pgpass == NULL) {
        conn->pgpass = strdup(DefaultPassword);
        if (!conn->pgpass) goto oom_error;
      } else
  ...
  if (conn->pghost && is_absolute_path(conn->pghost)) {
    if (conn->pgunixsocket) free(conn->pgunixsocket);
  ...
  if (conn->sslmode) {
    ...
    switch (conn->sslmode[0]) {
      ...
      case 'v': /* "verify-ca" or "verify-full" */
        conn->status = CONNECTION_BAD;
        printfPQExpBuffer(&conn->errorMessage,
                          libpq_gettext("sslmode value \"%s\" invalid when SSL "
    ...
    if (!conn->sslmode) goto oom_error;
  }
  ...
  if (conn->client_encoding_initial &&
    ...
    conn->client_encoding_initial =
        strdup(pg_encoding_to_char(pg_get_encoding_from_locale(NULL, true)));
    if (!conn->client_encoding_initial) goto oom_error;
  }
...
PGconn *PQsetdbLogin(const char *pghost, const char *pgport,
  ...
  if (pghost && pghost[0] != '\0') {
    ...
    if (!conn->pghost) goto oom_error;
  }

  if (pgport && pgport[0] != '\0') {
    if (conn->pgport) free(conn->pgport);
    conn->pgport = strdup(pgport);
    if (!conn->pgport) goto oom_error;
  }

  if (pgoptions && pgoptions[0] != '\0') {
    if (conn->pgoptions) free(conn->pgoptions);
    conn->pgoptions = strdup(pgoptions);
    if (!conn->pgoptions) goto oom_error;
  }

  if (pgtty && pgtty[0] != '\0') {
    if (conn->pgtty) free(conn->pgtty);
    conn->pgtty = strdup(pgtty);
    if (!conn->pgtty) goto oom_error;
  }

  if (login && login[0] != '\0') {
    if (conn->pguser) free(conn->pguser);
    conn->pguser = strdup(login);
    if (!conn->pguser) goto oom_error;
  }

  if (pwd && pwd[0] != '\0') {
    if (conn->pgpass) free(conn->pgpass);
   ...
   * Connect to the database
   */
  if (connectDBStart(conn)) (void)connectDBComplete(conn);
...
static int connectNoDelay(PGconn *conn) {
  ...
  if (setsockopt(conn->sock, IPPROTO_TCP, TCP_NODELAY, (char *)&on,
                 sizeof(on)) < 0) {
    char sebuf[256];
...
static void connectFailureMessage(PGconn *conn, int errorno) {
  ...
  if (IS_AF_UNIX(conn->raddr.addr.ss_family)) {
    ...
    pg_getnameinfo_all(&conn->raddr.addr, conn->raddr.salen, NULL, 0, service,
...
#endif /* HAVE_UNIX_SOCKETS */
  {
    ...
    if (conn->pghostaddr != NULL)
      ...
      if (pq_inet_net_ntop(AF_INET, &((struct sockaddr_in *)addr)->sin_addr.s_addr,
                        32, host_addr, sizeof(host_addr)) == NULL)
        strcpy(host_addr, "???");
    }
#ifdef HAVE_IPV6
    else if (addr->ss_family == AF_INET6) {
      if (pq_inet_net_ntop(AF_INET6,
                        &((struct sockaddr_in6 *)addr)->sin6_addr.s6_addr, 128,
    ...
    if ((conn->pghostaddr == NULL) &&
      ...
      appendPQExpBuffer(
          ...
          SOCK_STRERROR(errorno, sebuf, sizeof(sebuf)), displayed_host,
          host_addr, conn->pgport);
    else
      appendPQExpBuffer(
          &conn->errorMessage,
          libpq_gettext("could not connect to server: %s\n"
...
static int useKeepalives(PGconn *conn) {
  ...
  if (conn->keepalives == NULL) return 1;
...
static int setKeepalivesIdle(PGconn *conn) {
  ...
  if (conn->keepalives_idle == NULL) return 1;

  idle = atoi(conn->keepalives_idle);
  if (idle < 0) idle = 0;

#ifdef PG_TCP_KEEPALIVE_IDLE
  if (setsockopt(conn->sock, IPPROTO_TCP, PG_TCP_KEEPALIVE_IDLE, (char *)&idle,
                 sizeof(idle)) < 0) {
    char sebuf[256];
...
static int setKeepalivesInterval(PGconn *conn) {
  ...
  if (conn->keepalives_interval == NULL) return 1;

  interval = atoi(conn->keepalives_interval);
  if (interval < 0) interval = 0;

#ifdef TCP_KEEPINTVL
  if (setsockopt(conn->sock, IPPROTO_TCP, TCP_KEEPINTVL, (char *)&interval,
                 sizeof(interval)) < 0) {
    char sebuf[256];

    appendPQExpBuffer(
        &conn->errorMessage, libpq_gettext("setsockopt(%s) failed: %s\n"),
...
static int setKeepalivesCount(PGconn *conn) {
  ...
  if (conn->keepalives_count == NULL) return 1;

  count = atoi(conn->keepalives_count);
  if (count < 0) count = 0;

#ifdef TCP_KEEPCNT
  if (setsockopt(conn->sock, IPPROTO_TCP, TCP_KEEPCNT, (char *)&count,
                 sizeof(count)) < 0) {
    char sebuf[256];
...
static int setKeepalivesWin32(PGconn *conn) {
  ...
  if (conn->keepalives_idle) idle = atoi(conn->keepalives_idle);
  if (idle <= 0) idle = 2 * 60 * 60; /* 2 hours = default */

  if (conn->keepalives_interval) interval = atoi(conn->keepalives_interval);
  if (interval <= 0) interval = 1; /* 1 second = default */

  ka.onoff = 1;
  ka.keepalivetime = idle * 1000;
  ka.keepaliveinterval = interval * 1000;

  if (WSAIoctl(conn->sock, SIO_KEEPALIVE_VALS, (LPVOID)&ka, sizeof(ka), NULL, 0,
               &retsize, NULL, NULL) != 0) {
    appendPQExpBuffer(
        &conn->errorMessage,
...
static int connectDBStart(PGconn *conn) {
  ...
  if (ret || !addrs) {
    if (node)
      ...
      appendPQExpBuffer(&conn->errorMessage,
                        libpq_gettext("could not translate Unix-domain socket "
                                      "path \"%s\" to address: %s\n"),
 ...
 *
 *	 o	If you call PQtrace, ensure that the stream object into which
 *you trace will not block. o	If you do not supply an IP address for the
 *remote host (i.e. you supply a host name instead) then PQconnectStart will
 *block on gethostbyname.  You will be fine if using Unix sockets (i.e. by
...
PostgresPollingStatusType PQconnectPoll(PGconn *conn) {
  ...
  if (conn == NULL) return PGRES_POLLING_FAILED;
...
keep_going: /* We will come back to here until there is
  ...
  switch (conn->status) {
...
#ifndef WIN32
          else if (setsockopt(conn->sock, SOL_SOCKET, SO_KEEPALIVE, (char *)&on,
                              sizeof(on)) < 0) {
            appendPQExpBuffer(&conn->errorMessage,
                              libpq_gettext("setsockopt(%s) failed: %s\n"),
                              "SO_KEEPALIVE",
                              SOCK_STRERROR(SOCK_ERRNO, sebuf, sizeof(sebuf)));
            err = 1;
          } else if (!setKeepalivesIdle(conn) || !setKeepalivesInterval(conn) ||
...
#ifdef SO_NOSIGPIPE
        optval = 1;
        if (setsockopt(conn->sock, SOL_SOCKET, SO_NOSIGPIPE, (char *)&optval,
                       sizeof(optval)) == 0) {
    ...
    case CONNECTION_STARTED: {
      ...
      if (getsockopt(conn->sock, SOL_SOCKET, SO_ERROR, (char *)&optval,
                     &optlen) == -1) {
        appendPQExpBuffer(
            &conn->errorMessage,
      ...
      if (getsockname(conn->sock, (struct sockaddr *)&conn->laddr.addr,
                      &conn->laddr.salen) < 0) {
        appendPQExpBuffer(
            &conn->errorMessage,
    ...
    case CONNECTION_MADE: {
      ...
      if (conn->requirepeer && conn->requirepeer[0] &&
        ...
        if (getpeereid(conn->sock, &uid, &gid) != 0) {
          ...
          if (errno == ENOSYS)
            appendPQExpBufferStr(&conn->errorMessage,
                                 libpq_gettext("requirepeer parameter is not "
        ...
        if (pass == NULL) {
          if (passerr != 0)
            appendPQExpBuffer(
                ...
                (int)uid, pqStrerror(passerr, sebuf, sizeof(sebuf)));
          else
            appendPQExpBuffer(
                &conn->errorMessage,
        ...
        if (strcmp(pass->pw_name, conn->requirepeer) != 0) {
          appendPQExpBuffer(&conn->errorMessage,
                            libpq_gettext("requirepeer specifies \"%s\", but "
                                          "actual peer user name is \"%s\"\n"),
      ...
      if (PG_PROTOCOL_MAJOR(conn->pversion) >= 3)
        startpacket =
            pqBuildStartupPacket3(conn, &packetlen, EnvironmentOptions);
      else
        startpacket =
            pqBuildStartupPacket2(conn, &packetlen, EnvironmentOptions);
      if (!startpacket) {
        /*
    ...
    case CONNECTION_SSL_STARTUP: {
      ...
      if (!conn->ssl_in_use) {
        ...
        if (SSLok == 'S') {
          ...
          if (pqsecure_initialize(conn) != 0) goto error_return;
        } else if (SSLok == 'N') {
          /* mark byte consumed */
          conn->inStart = conn->inCursor;
    ...
    case CONNECTION_AWAITING_RESPONSE: {
      ...
      if (beresp == 'E') {
         ...
         */
        if (conn->sslmode[0] == 'p'                    /* "prefer" */
            && conn->ssl_in_use && conn->allow_ssl_try /* redundant? */
            && !conn->wait_ssl_try)                    /* redundant? */
        {
          /* only retry once */
          conn->allow_ssl_try = false;
      ...
      if (areq == AUTH_REQ_GSS_CONT) {
        ...
        if (llen != conn->ginbuf.length) {
          if (conn->ginbuf.value) free(conn->ginbuf.value);
...
static PGPing internal_ping(PGconn *conn) {
  ...
  if (!conn || !conn->options_valid) return PQPING_NO_ATTEMPT;

  /* Attempt to complete the connection */
  if (conn->status != CONNECTION_BAD) (void)connectDBComplete(conn);

  /* Definitely OK if we succeeded */
  if (conn->status != CONNECTION_BAD) return PQPING_OK;
   ...
   */
  if (conn->auth_req_received) return PQPING_OK;
   ...
   */
  if (strlen(conn->last_sqlstate) != 5) return PQPING_NO_RESPONSE;
...
static PGconn *makeEmptyPGconn(void) {
  ...
  if (!wsastartup_done) {
    ...
    if (WSAStartup(MAKEWORD(1, 1), &wsaData) != 0) return NULL;
...
static void freePGconn(PGconn *conn) {
  ...
  if (conn->client_encoding_initial) free(conn->client_encoding_initial);
  if (conn->events) free(conn->events);
  if (conn->pghost) free(conn->pghost);
  if (conn->pghostaddr) free(conn->pghostaddr);
  if (conn->pgport) free(conn->pgport);
  if (conn->pgunixsocket) free(conn->pgunixsocket);
  if (conn->pgtty) free(conn->pgtty);
  if (conn->connect_timeout) free(conn->connect_timeout);
  if (conn->pgoptions) free(conn->pgoptions);
  if (conn->appname) free(conn->appname);
  if (conn->fbappname) free(conn->fbappname);
  if (conn->dbName) free(conn->dbName);
  if (conn->replication) free(conn->replication);
  if (conn->pguser) free(conn->pguser);
  if (conn->pgpass) free(conn->pgpass);
  if (conn->keepalives) free(conn->keepalives);
  if (conn->keepalives_idle) free(conn->keepalives_idle);
  if (conn->keepalives_interval) free(conn->keepalives_interval);
  if (conn->keepalives_count) free(conn->keepalives_count);
  if (conn->sslmode) free(conn->sslmode);
  if (conn->sslcert) free(conn->sslcert);
  if (conn->sslkey) free(conn->sslkey);
  if (conn->sslrootcert) free(conn->sslrootcert);
  if (conn->sslcrl) free(conn->sslcrl);
  if (conn->sslcompression) free(conn->sslcompression);
  if (conn->requirepeer) free(conn->requirepeer);
  if (conn->krbsrvname) free(conn->krbsrvname);
  if (conn->gsslib) free(conn->gsslib);
  /* Note that conn->Pfdebug is not ours to close or free */
  if (conn->last_query) free(conn->last_query);
  if (conn->inBuffer) free(conn->inBuffer);
  if (conn->outBuffer) free(conn->outBuffer);
  if (conn->rowBuf) free(conn->rowBuf);
  if (conn->instance) free(conn->instance);
  if (conn->mode) free(conn->mode);
  if (conn->conntype) free(conn->conntype);
  if (conn->token) free(conn->token);
  if (conn->conn_id) free(conn->conn_id);
  termPQExpBuffer(&conn->errorMessage);
  termPQExpBuffer(&conn->workBuffer);
  if (conn->aeBuffer) {
    termAEParamers(conn->aeBuffer);
    conn->aeBuffer = NULL;
  }
  free(conn);
}
...
PGcancel *PQgetCancel(PGconn *conn) {
  ...
  if (conn->sock == PGINVALID_SOCKET) return NULL;
...
static int internal_cancel(SockAddr *raddr, int be_pid, int be_key,
   ...
   * only kernel calls.
   */
  if ((tmpsock = socket(raddr->addr.ss_family, SOCK_STREAM, 0)) ==
      PGINVALID_SOCKET) {
...
cancel_errReturn:
  ...
  if (tmpsock != PGINVALID_SOCKET) closesocket(tmpsock);
  SOCK_ERRNO_SET(save_errno);
  return FALSE;
}
...
int PQcancel(PGcancel *cancel, char *errbuf, int errbufsize) {
  ...
  return internal_cancel(&cancel->raddr, cancel->be_pid, cancel->be_key, errbuf,
                         errbufsize);
}
...
int PQrequestCancel(PGconn *conn) {
  ...
  if (!r) conn->errorMessage.len = strlen(conn->errorMessage.data);
 ...
 */
int pqPacketSend(PGconn *conn, char pack_type, const void *buf,
                 size_t buf_len) {
  /* Start the message. */
...
static int ldapServiceLookup(const char *purl, PQconninfoOption *options,
...
#endif
  bool found_keyword;
  char *url, *hostname, *portstr, *endptr, *dn, *scopestr, *filter, *result, *p,
      *p1 = NULL, *optname = NULL, *optval = NULL;
  ...
  *p = '\0';
  ...
  /* scope */
  if ((p = strchr(attrs[0], '?')) == NULL || *(p + 1) == '\0' ||
      *(p + 1) == '?') {
    printfPQExpBuffer(
        errorMessage,
        libpq_gettext(
  ...
  *p = '\0';
  ...
  /* filter */
  if ((p = strchr(scopestr, '?')) == NULL || *(p + 1) == '\0' ||
      *(p + 1) == '?') {
  ...
  *p = '\0';
  ...
  if ((p1 = strchr(hostname, ':')) != NULL) {
    ...
    *p1 = '\0';
    ...
    if (*portstr == '\0' || *endptr != '\0' || errno || lport < 0 ||
        lport > 65535) {
      printfPQExpBuffer(
          errorMessage,
  ...
  for (i = 0; values[i] != NULL; i++) size += values[i]->bv_len + 1;
...
static int parseServiceInfo(PQconninfoOption *options,
   ...
   */
  if (service == NULL) service = getenv("PGSERVICE");
  ...
  if ((env = getenv("PGSERVICEFILE")) != NULL)
    ...
    if (!pqGetHomeDirectory(homedir, sizeof(homedir))) goto next_file;
    snprintf(serviceFile, MAXPGPATH, "%s/%s", homedir, ".pg_service.conf");
    if (stat(serviceFile, &stat_buf) != 0) goto next_file;
  }

  status = parseServiceFile(serviceFile, service, options, errorMessage,
                            &group_found);
  if (group_found || status != 0) return status;
...
next_file:
  ...
  if (stat(serviceFile, &stat_buf) != 0) goto last_file;
...
static int parseServiceFile(const char *serviceFile, const char *service,
  ...
  if (f == NULL) {
    printfPQExpBuffer(errorMessage,
                      libpq_gettext("service file \"%s\" not found\n"),
                      serviceFile);
    return 1;
  }
  ...
  while ((line = fgets(buf, sizeof(buf), f)) != NULL) {
    ...
    while (*line && isspace((unsigned char)line[0])) line++;
      ...
      if (*group_found) {
        ...
        *val++ = '\0';
        ...
        if (strcmp(key, "service") == 0) {
          printfPQExpBuffer(
              errorMessage,
              libpq_gettext("nested service specifications not supported in "
                            "service file \"%s\", line %d\n"),
              serviceFile, linenr);
          fclose(f);
          return 3;
        }
        ...
        for (i = 0; options[i].keyword; i++) {
          if (strcmp(options[i].keyword, key) == 0) {
            if (options[i].val == NULL) options[i].val = strdup(val);
            if (!options[i].val) {
              printfPQExpBuffer(errorMessage, libpq_gettext("out of memory\n"));
              fclose(f);
              return 3;
  ...
  while (expand_dbname && keywords[i]) {
    ...
    if (strcmp(pname, "dbname") == 0 && pvalue) {
      ...
      if (recognized_connection_string(pvalue)) {
        ...
        if (dbname_options == NULL) return NULL;
      }
      break;
    }
    ++i;
  ...
  while (keywords[i]) {
    ...
    if (pvalue != NULL && pvalue[0] != '\0') {
      ...
      for (option = options; option->keyword != NULL; option++) {
        if (strcmp(option->keyword, pname) == 0) break;
      }
      ...
      if (strcmp(pname, "dbname") == 0 && dbname_options) {
        ...
        for (str_option = dbname_options; str_option->keyword != NULL;
             str_option++) {
          if (str_option->val != NULL) {
            ...
            for (k = 0; options[k].keyword; k++) {
              if (strcmp(options[k].keyword, str_option->keyword) == 0) {
                if (options[k].val) free(options[k].val);
...
static bool conninfo_add_defaults(PQconninfoOption *options,
   ...
   */
  for (option = options; option->keyword != NULL; option++) {
    if (option->val != NULL) continue; /* Value was in conninfo or service */
...
static bool conninfo_uri_parse_options(PQconninfoOption *options,
  ...
  if (*p == '@') {
    ...
    *p = '\0';
    ...
    if (*user &&
        !conninfo_storeval(options, "user", user, errorMessage, false, true))
      goto cleanup;
    ...
    if (prevchar == ':') {
      ...
      *p = '\0';
      ...
      if (*password && !conninfo_storeval(options, "password", password,
  ...
  if (*p == '[') {
    ...
    if (!*p) {
      printfPQExpBuffer(
          errorMessage,
          libpq_gettext("end of string reached when looking for matching \"]\" "
    ...
    *(p++) = '\0';
    ...
    if (*p && *p != ':' && *p != '/' && *p != '?') {
      printfPQExpBuffer(
          errorMessage,
          libpq_gettext("unexpected character \"%c\" at position %d in URI "
                        "(expected \":\" or \"/\"): \"%s\"\n"),
          *p, (int)(p - buf + 1), uri);
      goto cleanup;
    }
  ...
  *p = '\0';
  ...
  if (*host &&
      !conninfo_storeval(options, "host", host, errorMessage, false, true))
    goto cleanup;
  ...
  if (prevchar == ':') {
    ...
    *p = '\0';
    ...
    if (*port &&
        !conninfo_storeval(options, "port", port, errorMessage, false, true))
      goto cleanup;
  }
  ...
  if (prevchar && prevchar != '?') {
    ...
    *p = '\0';
     ...
     * it at all.
     */
    if (*dbname && !conninfo_storeval(options, "dbname", dbname, errorMessage,
                                      false, true))
      goto cleanup;
  }
  ...
  if (prevchar) {
    ...
    if (!conninfo_uri_parse_params(p, options, errorMessage)) goto cleanup;
  }
  ...
  while (*params) {
    ...
    for (;;) {
      if (*p == '=') {
        ...
        if (value != NULL) {
          printfPQExpBuffer(errorMessage,
                            libpq_gettext("extra key/value separator \"=\" in "
                                          "URI query parameter: \"%s\"\n"),
                            keyword);
          return false;
        }
        ...
        *p++ = '\0';
        ...
        if (value == NULL) {
          printfPQExpBuffer(errorMessage,
                            libpq_gettext("missing key/value separator \"=\" "
     ...
     * URI-encoded.
     */
    if (!conninfo_storeval(connOptions, keyword, value, errorMessage, true,
                           false)) {
 ...
 */
static PQconninfoOption *conninfo_storeval(
    PQconninfoOption *connOptions, const char *keyword, const char *value,
...
static PQconninfoOption *conninfo_find(PQconninfoOption *connOptions,
  ...
  for (option = connOptions; option->keyword != NULL; option++) {
    if (strcmp(option->keyword, keyword) == 0) return option;
  }
...
PQconninfoOption *PQconninfo(PGconn *conn) {
  ...
  if (connOptions != NULL) {
    ...
    for (option = PQconninfoOptions; option->keyword; option++) {
      ...
      if (option->connofs < 0) continue;

      connmember = (char **)((char *)conn + option->connofs);

      if (*connmember)
        conninfo_storeval(connOptions, option->keyword, *connmember, &errorBuf,
                          true, false);
    }
  }
...
void PQconninfoFree(PQconninfoOption *connOptions) {
  ...
  for (option = connOptions; option->keyword != NULL; option++) {
    if (option->val != NULL) free(option->val);
  }
  free(connOptions);
}
...
PGTransactionStatusType PQtransactionStatus(const PGconn *conn) {
  if (!conn || conn->status != CONNECTION_OK) return PQTRANS_UNKNOWN;
  if (conn->asyncStatus != PGASYNC_IDLE) return PQTRANS_ACTIVE;
...
const char *PQparameterStatus(const PGconn *conn, const char *paramName) {
  ...
  if (!conn || !paramName) return NULL;
  for (pstatus = conn->pstatus; pstatus != NULL; pstatus = pstatus->next) {
    if (strcmp(pstatus->name, paramName) == 0) return pstatus->value;
  }
  return NULL;
}

char *AEparameters(const PGconn *conn, const char *paramName) {
  const aeParameters *pstatus;

  if (!conn || !paramName) return NULL;
  for (pstatus = conn->aeBuffer; pstatus != NULL; pstatus = pstatus->next) {
    if (strncmp(pstatus->key, paramName, strlen(pstatus->key)) == 0)
...
int PQbackendPID(const PGconn *conn) {
  if (!conn || conn->status != CONNECTION_OK) return 0;
  return conn->be_pid;
}
...
int PQsetClientEncoding(PGconn *conn, const char *encoding) {
  ...
  if (sizeof(qbuf) < (sizeof(query) + strlen(encoding))) return -1;
...
PGVerbosity PQsetErrorVerbosity(PGconn *conn, PGVerbosity verbosity) {
  ...
  if (!conn) return PQERRORS_DEFAULT;
  old = conn->verbosity;
...
PQnoticeReceiver PQsetNoticeReceiver(PGconn *conn, PQnoticeReceiver proc,
                                     void *arg) {
...
PQnoticeProcessor PQsetNoticeProcessor(PGconn *conn, PQnoticeProcessor proc,
                                       void *arg) {
...
static char *pwdfMatchesString(char *buf, char *token) {
  ...
  if (buf == NULL || token == NULL) return NULL;
  tbuf = buf;
  ttok = token;
  if (tbuf[0] == '*' && tbuf[1] == ':') return tbuf + 2;
  while (*tbuf != 0) {
    if (*tbuf == '\\' && !bslash) {
      tbuf++;
      bslash = true;
    }
    if (*tbuf == ':' && *ttok == 0 && !bslash) return tbuf + 1;
    bslash = false;
    if (*ttok == 0) return NULL;
    if (*tbuf == *ttok) {
      tbuf++;
      ttok++;
    } else
...
static char *PasswordFromFile(char *hostname, char *port, char *dbname,
  ...
  if (dbname == NULL || strlen(dbname) == 0) return NULL;

  if (username == NULL || strlen(username) == 0) return NULL;
  ...
  if (hostname == NULL)
     ...
     */
    if (strcmp(hostname, DEFAULT_PGSOCKET_DIR) == 0) hostname = DefaultHost;

  if (port == NULL) port = DEF_PGPORT_STR;

  if (!getPgPassFilename(pgpassfile)) return NULL;

  /* If password file cannot be opened, ignore it. */
  if (stat(pgpassfile, &stat_buf) != 0) return NULL;
  ...
  if (stat_buf.st_mode & (S_IRWXG | S_IRWXO)) {
    fprintf(
        stderr,
        libpq_gettext("WARNING: password file \"%s\" has group or world "
  ...
  while (!feof(fp) && !ferror(fp)) {
    ...
    if (!enlargePQExpBuffer(&buf, 128)) break;

    /* Read some data, appending it to what we already have */
    if (fgets(buf.data + buf.len, buf.maxlen - buf.len, fp) == NULL) break;
    ...
    if (buf.data[0] != '#') {
      ...
      if (len > 0 && t[len - 1] == '\n') {
        ...
        if (len > 0 && t[len - 1] == '\r') t[--len] = '\0';
      }

      if (len > 0 && (t = pwdfMatchesString(t, hostname)) != NULL &&
...
static bool getPgPassFilename(char *pgpassfile) {
  ...
  if ((passfile_env = getenv("PGPASSFILE")) != NULL)
    ...
    if (!pqGetHomeDirectory(homedir, sizeof(homedir))) return false;
...
static void dot_pg_pass_warning(PGconn *conn) {
  ...
  if (conn->dot_pgpass_used && conn->password_needed && conn->result &&
    ...
    if (!getPgPassFilename(pgpassfile)) return;
    appendPQExpBuffer(&conn->errorMessage,
