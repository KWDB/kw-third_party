...
struct pg_conn {
  ...
  aeParameters *aeBuffer;
  char* token; /* inner connect use this to authentication */
  char* conntype; /* 0: normal conn; 1: inner conn; 2: heartbeat */
};

typedef enum {
  CONNECT_NORMAL = 0,  // 0: normal conn
  CONNECT_INNER,   // 1: inner conn
  CONNECT_HEARTBEAT,  // 2: heartbeat
  CONNECT_KMM,  // 3: kmm should replace inner conn later
  CONNECT_REGISTER, // 4: register token
  CONNECT_MAX_TYPE
} ConnectType;
...
extern int pqPacketSend(PGconn *conn, char pack_type, const void *buf,
                        size_t buf_len);
...
extern PGresult *pqFunctionCall2(PGconn *conn, Oid fnid, int *result_buf,
...
extern PGresult *pqFunctionCall3(PGconn *conn, Oid fnid, int *result_buf,
                                 int *actual_result_len, int result_is_int,
                                 const PQArgBlock *args, int nargs);
...
extern char *libpq_ngettext(const char *msgid, const char *msgid_plural,
                            unsigned long n) pg_attribute_format_arg(1)
