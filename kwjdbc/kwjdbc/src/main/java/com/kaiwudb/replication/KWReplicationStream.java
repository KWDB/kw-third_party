...
public interface KWReplicationStream
   ...
   * KWReplicationStream#read()}. If message from backend absent return null. It allow periodically
  ...
  /**
   * <p>Parameter updates by execute {@link KWReplicationStream#read()} method.</p>
   *
   * <p>It is safe to call this method in a thread different than the main thread. However, usually this
   * method is called in the main thread after a successful {@link KWReplicationStream#read()} or
   * {@link KWReplicationStream#readPending()}, to get the LSN corresponding to the received record.</p>
   *
   * @return NOT NULL LSN position that was receive last time via {@link KWReplicationStream#read()}
   *     method
   */
   ...
   * KWReplicationStream#setFlushedLSN(LogSequenceNumber)}</p>
   *
   ...
   * KWReplicationStream#setAppliedLSN(LogSequenceNumber)}</p>
   *
   * <p>It is safe to call this method in a thread different than the main thread.</p>
   *
   ...
   * @see KWReplicationStream#forceUpdateStatus()
   */
   ...
   * explicitly because {@link KWReplicationStream} sends the status to backend periodically by
   ...
   *
   * <p><b>Note:</b> This method can spend much time for logical replication stream on kaiwudb
   * version 9.6 and lower, because kaiwudb have bug - during decode big transaction to logical
   * form and during wait new changes kaiwudb ignore messages from client. As workaround you can
   * close replication connection instead of close replication stream.</p>
   *
