...
import com.kaiwudb.ssl.NonValidatingFactory.NonValidatingTM;
...
public class LibPQFactory extends WrappedFactory {
  ...
  private CallbackHandler getCallbackHandler(Properties info) throws KSQLException {
    ...
    CallbackHandler cbh;
    String sslpasswordcallback = KWProperty.SSL_PASSWORD_CALLBACK.get(info);
    if (sslpasswordcallback != null) {
      ...
      cbh = new ConsoleCallbackHandler(KWProperty.SSL_PASSWORD.get(info));
    }
    return cbh;
  }
  ...
  private void initPk8(String sslkeyfile, String defaultdir, Properties info) throws KSQLException {
    ...
    String sslcertfile = KWProperty.SSL_CERT.get(info);
    if (sslcertfile == null) { // Fall back to default
      defaultfile = true;
      sslcertfile = defaultdir + "kaiwudb.crt";
    }
   ...
   * @throws KSQLException if security error appears when initializing factory
   */
  public LibPQFactory(Properties info) throws KSQLException {
    try {
      ...
      if (System.getProperty("os.name").toLowerCase().contains("windows")) { // It is Windows
        defaultdir = System.getenv("APPDATA") + pathsep + "kaiwudb" + pathsep;
      } else {
        defaultdir = System.getProperty("user.home") + pathsep + ".kaiwudb" + pathsep;
      }

      String sslkeyfile = KWProperty.SSL_KEY.get(info);
      if (sslkeyfile == null) { // Fall back to default
        defaultfile = true;
        sslkeyfile = defaultdir + "kaiwudb.pk8";
      }
      ...
      if (!sslMode.verifyCertificate()) {
        ...
        String sslrootcertfile = KWProperty.SSL_ROOT_CERT.get(info);
      ...
      try {
        ...
        throw new KSQLException(GT.tr("Could not initialize SSL context."),
            KSQLState.CONNECTION_FAILURE, ex);
      }

      factory = ctx.getSocketFactory();
    } catch (NoSuchAlgorithmException ex) {
      throw new KSQLException(GT.tr("Could not find a java cryptographic algorithm: {0}.",
              ex.getMessage()), KSQLState.CONNECTION_FAILURE, ex);
    }
  }
   ...
   *
   * @throws KSQLException if there is an exception to propagate
   */
