  ...
  public ServerErrorMessage(EncodingPredictor.DecodeResult serverError) {
    ...
    if (serverError.encoding != null) {
      mesgParts.put(MESSAGE, mesgParts.get(MESSAGE)
          + GT.tr(" (kwjdbc: autodetected server-encoding to be {0}, if the message is not readable, please check database logs and/or host, port, dbname, user, password, kw_hba.conf)",
