...
/**
 * This interface defines the public KaiwuDB extensions to java.sql.Connection. All Connections
 * returned by the KaiwuDB driver implement KWConnection.
 */
public interface KWConnection {
   ...
   * @since 7.3
   */
  KWNotification[] getNotifications() throws SQLException;
   ...
   * @since 43
   */
  KWNotification[] getNotifications(int timeoutMillis) throws SQLException;
  ...
  /**
   * This allows client code to add a handler for one of com.kaiwudb's more unique data types. It
  ...
  /**
   * <p>This allows client code to add a handler for one of com.kaiwudb's more unique data types.</p>
   *
   ...
   * <pre>
   * ...
   * ((KWConnection)myconn).addDataType("mytype", my.class.name.class);
   * ...
   * </pre>
   *
   * <p>where myconn is an open Connection to com.kaiwudb.</p>
   *
   * <p>The handling class must extend KWobject</p>
   *
   * @param type the KaiwuDB type to register
   * @param klass the class implementing the Java representation of the type; this class must
   *        implement {@link KWobject}).
   * @throws SQLException if <code>klass</code> does not implement
   *         {@link KWobject}).
   * @see KWobject
   * @since 8.0
   */
  void addDataType(String type, Class<? extends KWobject> klass) throws SQLException;

  /**
   * Set the default statement reuse threshold before enabling server-side prepare. See
   * {@link KWStatement#setPrepareThreshold(int)} for details.
   *
   ...
   */
  KWReplicationConnection getReplicationAPI();

  /**
   * <p>Returns the current values of all parameters reported by the server.</p>
   *
   * <p>KaiwuDB reports values for a subset of parameters (GUCs) to the client
   * at connect-time, then sends update messages whenever the values change
   * during a session. KwJDBC records the latest values and exposes it to client
   * applications via <code>getParameterStatuses()</code>.</p>
   *
   * <p>KwJDBC exposes individual accessors for some of these parameters as
   ...
   * <code>GUC_REPORT</code> in the source code. The <code>kw_settings</code>
   * view does not expose information about which parameters are reportable.
   * KwJDBC's map will only contain the parameters the server reports values
   ...
   * in this map, like in KaiwuDB itself. So <code>DateStyle</code> and
   * <code>datestyle</code> are the same key.</p>
   *
   * <p>
   *  As of KaiwuDB 11 the reportable parameter list, and related KwJDBC
   ...
   *  </li>
   *  <li>
   *    <code>client_encoding</code> - KwJDBC always sets this to <code>UTF8</code>.
   *    See <code>allowEncodingChanges</code> connection property.
   *  </li>
   *  <li><code>DateStyle</code> - KwJDBC requires this to always be set to <code>ISO</code></li>
   ...
   *    and {@link java.util.TimeZone#setDefault(java.util.TimeZone)}
   *  </li>
   ...
   * </ul>
   *
   * <p>Note that some KwJDBC operations will change server parameters
   * automatically.</p>
   *
