...
public class Utils {
  ...
  public static byte[] encodeUTF8(String str) {
    // See com.kaiwudb.benchmark.encoding.UTF8Encoding#string_getBytes
    ...
    try {
      if (standardConformingStrings) {
        ...
        for (int i = 0; i < value.length(); ++i) {
          ...
          if (ch == '\0') {
            throw new KSQLException(GT.tr("Zero bytes may not occur in string parameters."),
      ...
      throw new KSQLException(GT.tr("No IOException expected from StringBuffer or StringBuilder"),
  ...
  private static void doAppendEscapedIdentifier(Appendable sbuf, String value) throws SQLException {
    try {
      ...
      for (int i = 0; i < value.length(); ++i) {
        char ch = value.charAt(i);
        if (ch == '\0') {
          throw new KSQLException(GT.tr("Zero bytes may not occur in identifiers."),
              KSQLState.INVALID_PARAMETER_VALUE);
        }
        if (ch == '"') {
          sbuf.append(ch);
