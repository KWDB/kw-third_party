...
import com.kaiwudb.jdbc.EscapedFunctions2;
 ...
 * Note: This class should not be considered as kwjdbc public API.
 *
...
public class Parser {
  ...
  /**
   * Parses JDBC query into KaiwuDB's native format. Several queries might be given if separated
   * by semicolon.
   *
  ...
  public static boolean isOperatorChar(char c) {
     ...
     * in kwsql/src/backend/parser/scan.l.
     */
    return ",()[].;:+-*/%^<>=~!@#&|`?".indexOf(c) != -1;
  }
   ...
   * KaiwuDB 9.4 allows column names like _, ‿, ⁀, ⁔, ︳, ︴, ﹍, ﹎, ﹏, ＿, so
   * it is assumed isJavaIdentifierPart is good enough for KaiwuDB.
   *
  ...
  public static boolean isIdentifierStartChar(char c) {
    /*
     * KaiwuDB's implmementation is located in
     * kwsql/src/backend/parser/scan.l:
   ...
   * [?,..])] }} into the KaiwuDB format which is {@code select <some_function> (?, [?, ...]) as
  ...
  public static JdbcCallParseInfo modifyJdbcCall(String jdbcSql, boolean stdStrings,
    ...
    // RE: frequently used statements are cached (see {@link KwConnection#borrowQuery}), so this "merge" is not that important.
    String sql = jdbcSql;
    ...
    if (syntaxError) {
      throw new KSQLException(
          ...
          KSQLState.STATEMENT_NOT_ALLOWED_IN_FUNCTION_CALL);
    }
