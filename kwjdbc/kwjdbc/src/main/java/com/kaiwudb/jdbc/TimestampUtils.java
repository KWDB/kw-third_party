...
public class TimestampUtils {
  ...
  // since KaiwuDB has microsecond resolution only
  ...
  static {
    ...
    for (int i = -12; i <= 14; i++) {
      ...
      if (i == 0) {
        GMT_ZONES.put(kwZoneName, timeZone);
        continue;
      }
      GMT_ZONES.put(kwZoneName + Math.abs(i), timeZone);
      GMT_ZONES.put(kwZoneName + new String(NUMBERS[Math.abs(i)]), timeZone);
    }
  ...
  private ParsedTimestamp parseBackendTimestamp(String str) throws SQLException {
    ...
    try {
      ...
      throw new KSQLException(
          ...
          KSQLState.BAD_DATETIME_FORMAT, nfe);
    }
  ...
  public synchronized Timestamp toTimestamp(Calendar cal, String s) throws SQLException {
    ...
    // convert kaiwudb's infinity values to internal infinity magic value
    if (slen == 8 && s.equals("infinity")) {
      return new Timestamp(KWStatement.DATE_POSITIVE_INFINITY);
    }

    if (slen == 9 && s.equals("-infinity")) {
      return new Timestamp(KWStatement.DATE_NEGATIVE_INFINITY);
    }
   ...
   * @throws KSQLException If binary format could not be parsed.
   */
  ...
  public synchronized String toString(Calendar cal, Timestamp x,
    ...
    if (x.getTime() == KWStatement.DATE_POSITIVE_INFINITY) {
      return "infinity";
    } else if (x.getTime() == KWStatement.DATE_NEGATIVE_INFINITY) {
      return "-infinity";
    }
    ...
    if (nanos >= MAX_NANOS_BEFORE_WRAP_ON_ROUND) {
      ...
      // KaiwuDB does not support nanosecond resolution yet, and appendTime will just ignore
  ...
  /**
   * Appends time part to the {@code StringBuilder} in KaiwuDB-compatible format.
  ...
  public Date toDateBin(TimeZone tz, byte[] bytes) throws KSQLException {
    if (bytes.length != 4) {
      throw new KSQLException(GT.tr("Unsupported binary encoding of {0}.", "date"),
          KSQLState.BAD_DATETIME_FORMAT);
    }
    ...
    if (millis <= KWStatement.DATE_NEGATIVE_SMALLER_INFINITY) {
      millis = KWStatement.DATE_NEGATIVE_INFINITY;
    } else if (millis >= KWStatement.DATE_POSITIVE_SMALLER_INFINITY) {
      millis = KWStatement.DATE_POSITIVE_INFINITY;
    } else {
  ...
  public Time toTimeBin(TimeZone tz, byte[] bytes) throws KSQLException {
    if ((bytes.length != 8 && bytes.length != 12)) {
      throw new KSQLException(GT.tr("Unsupported binary encoding of {0}.", "time"),
          KSQLState.BAD_DATETIME_FORMAT);
    }
  ...
  private ParsedBinaryTimestamp toParsedTimestampBinPlain(byte[] bytes)
    ...
    if (bytes.length != 8) {
      throw new KSQLException(GT.tr("Unsupported binary encoding of {0}.", "timestamp"),
              KSQLState.BAD_DATETIME_FORMAT);
    }
  ...
  public Date convertToDate(long millis, TimeZone tz) {
    ...
    if (millis <= KWStatement.DATE_NEGATIVE_INFINITY
        || millis >= KWStatement.DATE_POSITIVE_INFINITY) {
      return new Date(millis);
    }
    if (tz == null) {
  ...
  /**
   * Returns the given time value as String matching what the current kaiwudb server would send
   * in text mode.
   *
  ...
  /**
   * Converts the given kaiwudb seconds to java seconds. Reverse engineered by inserting varying
   * dates to kaiwudb and tuning the formula until the java dates matched. See {@link #toKwSecs}
   * for the reverse operation.
   *
   * @param secs KaiwuDB seconds.
   * @return Java seconds.
   */
  ...
  /**
   * Converts the given java seconds to kaiwudb seconds. See {@link #toJavaSecs} for the reverse
   * operation. The conversion is valid for any year 100 BC onwards.
   *
   * @param secs KaiwuDB seconds.
   * @return Java seconds.
   */
  private static long toKwSecs(long secs) {
    // java epoc to kaiwudb epoc
    secs -= 946684800L;
  ...
  public void toBinDate(TimeZone tz, byte[] bytes, Date value) throws KSQLException {
    ...
    // See com.kaiwudb.jdbc.AbstractJdbc2Statement.setDate(int, java.sql.Date,
    // java.util.Calendar)
    // The problem is we typically do not know for sure what is the exact required date/timestamp
    // type
    // Thus kwjdbc sticks to text transfer.
    millis += tz.getOffset(millis);

    long secs = toKwSecs(millis / 1000);
