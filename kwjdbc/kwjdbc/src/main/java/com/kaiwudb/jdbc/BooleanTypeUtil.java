...
/**
 * <p>Helper class to handle boolean type of KaiwuDB.</p>
 *
 * <p>Based on values accepted by the KaiwuDB server:
 */
class BooleanTypeUtil {
   ...
   * @throws KSQLException PSQLState.CANNOT_COERCE
   */
  static boolean castToBoolean(final Object in) throws KSQLException {
  ...
  private static boolean fromString(final String strval) throws KSQLException {
  ...
  private static boolean fromCharacter(final Character charval) throws KSQLException {
  ...
  private static boolean fromNumber(final Number numval) throws KSQLException {
  ...
  private static KSQLException cannotCoerceException(final Object value) {
    if (LOGGER.isLoggable(Level.FINE)) {
      LOGGER.log(Level.FINE, "Cannot cast to boolean: \"{0}\"", String.valueOf(value));
    }
    return new KSQLException(GT.tr("Cannot cast to boolean: \"{0}\"", String.valueOf(value)),
        KSQLState.CANNOT_COERCE);
  }
