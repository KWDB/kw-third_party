...
public class KwStatement implements Statement, BaseStatement {
  ...
  private static final boolean DEFAULT_FORCE_BINARY_TRANSFERS =
      Boolean.getBoolean("com.kaiwudb.forceBinary");
  ...
  private static final AtomicReferenceFieldUpdater<KwStatement, TimerTask> CANCEL_TIMER_UPDATER =
      AtomicReferenceFieldUpdater.newUpdater(KwStatement.class, TimerTask.class, "cancelTimerTask");
  ...
  private static final AtomicReferenceFieldUpdater<KwStatement, StatementCancelState> STATE_UPDATER =
      AtomicReferenceFieldUpdater.newUpdater(KwStatement.class, StatementCancelState.class, "statementState");
   ...
   * The warnings chain.
   */
  protected volatile KSQLWarningWrapper warnings = null;
  ...
  KwStatement(KwConnection c, int rsType, int rsConcurrency, int rsHoldability)
      throws SQLException {
    this.connection = c;
  ...
  public ResultSet createResultSet(Query originalQuery, Field[] fields, List<byte[][]> tuples,
    ...
    KwResultSet newResult = new KwResultSet(originalQuery, this, fields, tuples, cursor,
  ...
  public BaseConnection getKWConnection() {
    return connection;
  }
  ...
  public class StatementResultHandler extends ResultHandlerBase {
    ...
    public void handleResultRows(Query fromQuery, Field[] fields, List<byte[][]> tuples,
        ResultCursor cursor) {
      try {
        ResultSet rs = KwStatement.this.createResultSet(fromQuery, fields, tuples, cursor);
    ...
    public void handleWarning(SQLWarning warning) {
      KwStatement.this.addWarning(warning);
    }
  ...
  protected ResultSet getSingleResultSet() throws SQLException {
    synchronized (this) {
      ...
      if (result.getNext() != null) {
        throw new KSQLException(GT.tr("Multiple ResultSets were returned by the query."),
  ...
  public boolean executeWithFlags(int flags) throws SQLException {
    checkClosed();
    throw new KSQLException(GT.tr("Can''t use executeWithFlags(int) on a Statement."),
  ...
  protected void closeForNextExecution() throws SQLException {
    ...
    synchronized (this) {
      while (firstUnclosedResult != null) {
        KwResultSet rs = (KwResultSet)firstUnclosedResult.getResultSet();
        if (rs != null) {
          rs.closeInternally();
        }
  ...
  public void setQueryTimeoutMs(long millis) throws SQLException {
    ...
    if (millis < 0) {
      throw new KSQLException(GT.tr("Query timeout must be a value greater than or equals to 0."),
          KSQLState.INVALID_PARAMETER_VALUE);
    }
    timeout = millis;
  }
  ...
  public void addWarning(SQLWarning warn) {
    ...
    final KSQLWarningWrapper warnWrap = this.warnings;
    if (warnWrap == null) {
      this.warnings = new KSQLWarningWrapper(warn);
    } else {
  ...
  /*
   *
   * The following methods are kaiwudb extensions and are defined in the interface BaseStatement
   *
   */
  ...
  protected void checkClosed() throws SQLException {
    if (isClosed()) {
      throw new KSQLException(GT.tr("This statement has been closed."),
  ...
  private void startTimer() {
    ...
    TimerTask cancelTask = new TimerTask() {
      ...
      public void run() {
        try {
          if (!CANCEL_TIMER_UPDATER.compareAndSet(KwStatement.this, this, null)) {
            // Nothing to do here, statement has already finished and cleared
            // cancelTimerTask reference
            return;
          }
          KwStatement.this.cancel();
        } catch (SQLException e) {
        }
      }
    };
  ...
  public long executeLargeUpdate(String sql, int[] columnIndexes) throws SQLException {
    ...
    throw new KSQLException(GT.tr("Returning autogenerated keys by column index is not supported."),
