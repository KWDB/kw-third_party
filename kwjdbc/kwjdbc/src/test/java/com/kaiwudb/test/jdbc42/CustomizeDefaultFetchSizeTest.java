...
public class CustomizeDefaultFetchSizeTest {
  ...
  public void testSetPredefineDefaultFetchSizeOnStatement() throws Exception {
    ...
    KWProperty.DEFAULT_ROW_FETCH_SIZE.set(properties, waitFetchSize);
    ...
    assertThat(
        "KWProperty.DEFAULT_ROW_FETCH_SIZE should be propagate to Statement that was create from connection "
  ...
  public void testSetPredefineDefaultFetchSizeOnPreparedStatement() throws Exception {
    ...
    assertThat(
        "KWProperty.DEFAULT_ROW_FETCH_SIZE should be propagate to CallableStatement that was create from connection "
            + "on that define it parameter",
        resultFetchSize, CoreMatchers.equalTo(waitFetchSize));
  }
  ...
  public void testNotAvailableSpecifyNegativeFetchSize() throws Exception {
    Properties properties = new Properties();
    KWProperty.DEFAULT_ROW_FETCH_SIZE.set(properties, Integer.MIN_VALUE);

    connection = TestUtil.openDB(properties);

    fail(
        "On step initialize connection we know about not valid parameter KWProperty.DEFAULT_ROW_FETCH_SIZE they can't be negative, "
