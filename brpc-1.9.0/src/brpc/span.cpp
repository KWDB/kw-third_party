...
#else
  int Index(const Span* span, std::string* value_buf) { return -1; }
  int RemoveSpansBefore(int64_t tm) { return -1; }
#endif
...
SpanDB* SpanDB::Open() {
  ...
  LOG(INFO) << "Opened " << local.id_db_name << " 和 "
            << local.time_db_name;
  Swap(local, *db);
  return db;
#else
  LOG(ERROR) << "LevelDB support disabled, unable to open SpanDB";
  return NULL;
#endif
}
...
#else
  // 非LevelDB模式下直接销毁Span，不执行索引
    LOG(INFO) << "LevelDB支持已禁用，直接销毁Span";
    destroy();
#endif
}
...
int FindSpan(uint64_t trace_id, uint64_t span_id, RpczSpan* response) {
  ...
  return 0;
#else
  LOG(ERROR) << "LevelDB support disabled, unable to find Span";
    return -1;
#endif
}
