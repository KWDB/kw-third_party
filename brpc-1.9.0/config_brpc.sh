...
TEMP=`getopt -o v: --long headers:,libs:,cc:,cxx:,with-glog,with-thrift,with-rdma,with-mesalink,with-leveldb,nodebugsymbols -n 'config_brpc' -- "$@"`
WITH_GLOG=0
WITH_THRIFT=0
WITH_RDMA=0
WITH_MESALINK=0
...
while true; do
    case "$1" in
        ...
        --with-leveldb) WITH_LEVELDB=1; shift 1 ;;
...
CPPFLAGS="-DBRPC_WITH_GLOG=$WITH_GLOG -DBRPC_WITH_LEVELDB=$WITH_LEVELDB -DGFLAGS_NS=$GFLAGS_NS"
...
#ifdef BRPC_WITH_LEVELDB
#undef BRPC_WITH_LEVELDB
#endif
#define BRPC_WITH_LEVELDB $WITH_LEVELDB
